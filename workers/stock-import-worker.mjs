...
try{
  ...
  const parsed=parseStockRows(rows,{maxRows:workerData.maxRows,onProgress:(processed,total)=>progress("Đang kiểm tra dữ liệu Stock",30+Math.round(processed/Math.max(1,total)*28),processed,total)}),partPath=workerData.resultPath+".part",handle=await fs.open(partPath,"w",0o600),{records,stockMetadata=[],...metadata}=parsed,metadataBySku=new Map(stockMetadata.map((item)=>[String(item.sku).toUpperCase(),item]));
  try{await handle.write(JSON.stringify(metadata)+"\n");for(let index=0;index<records.length;index+=2000){await handle.write(records.slice(index,index+2000).map((record)=>JSON.stringify({...metadataBySku.get(String(record.sku).toUpperCase()),...record})).join("\n")+"\n");progress("Đang chuẩn bị cập nhật tồn kho",60+Math.round(Math.min(records.length,index+2000)/Math.max(1,records.length)*8),Math.min(records.length,index+2000),records.length);}await handle.sync();}finally{await handle.close();}
