...
app.get("/api/store", async (req, res, next) => {
  try {
    ...
    const pickItem=(item)=>{const product=state.products.find((p)=>p.id===item.productId),assignee=accountsById.get(item.userId);return product?{...withUploadedStock(product,uploaded),pickId:asText(item.id,item.productId),quantity:Math.max(1,asInt(item.quantity,1)),picked:item.picked,available:item.available!==false,customerName:asText(item.customerName),customerPhone:normalizePhone(item.customerPhone),note:asText(item.note),assignedBy:asText(item.assignedBy),assigneeId:item.userId,assigneeName:asText(assignee?.name,"Nhân viên đã xóa"),orderDate:normalizeOrderDate(item.orderDate,item.createdAt||Date.now()),createdAt:asInt(item.createdAt,Date.now())}:null;};
    ...
    const picking=state.picking.filter((item)=>item.userId===actor.userId).map(pickItem).filter(Boolean),assignedPicking=(canManage(actor.role)?state.picking:state.picking.filter((item)=>item.userId===actor.userId)).map(pickItem).filter(Boolean),orderHistory=state.orderHistory.filter((item)=>canManage(actor.role)||item.userId===actor.userId).map(historyItem).filter(Boolean).slice(0,500);
...
app.get("/api/stock", async(req,res,next)=>{
  try {
    ...
    const rows=[];for(const record of state.stockRecords){const product=productsBySku.get(normalizeText(record.sku)),row={...(product||{id:"stock-"+record.sku,sku:record.sku,name:"SKU chưa có trong Master Data",line:"--",lineName:"",side:"",bay:0}),sales:record.sales??0,stock:record.stock,stockKnown:true,updatedAt:record.updatedAt};if(!query||productSearchText(row).includes(query))rows.push(row);}
...
app.get("/api/stock/export.csv",async(req,res,next)=>{
  try {
    ...
    res.write("\uFEFF"+["SKU","TÊN SẢN PHẨM","Sale","Stock","GIÁ BÁN RETAIL","GIÁ KHUYẾN MÃI","Division","DIVISION NAME","Department","DEPARTMENT NAME","BARCODE NCC","BARCODE AEON"].map(csvCell).join(",")+"\n");
    for(const record of state.stockRecords){const product=productsBySku.get(normalizeText(record.sku))||{},row=[record.sku,product.name,record.sales,record.stock,product.price??0,product.promoPrice??0,product.division,product.divisionName,product.department,product.departmentName,product.supplierBarcode,product.barcode||product.supplierBarcode].map(csvCell).join(",")+"\n";if(!res.write(row))await once(res,"drain");}
    res.end();
  } catch(error){next(error);}
});
