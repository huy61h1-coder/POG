...
app.get("/api/store", async (req, res, next) => {
  try {
    ...
    const pickItem=(item)=>{const product=state.products.find((p)=>p.id===item.productId),assignee=accountsById.get(item.userId),deliveryAssignee=accountsById.get(item.deliveryAssigneeId),workflowStatus=["unassigned","picking","ready_delivery","delivered"].includes(item.workflowStatus)?item.workflowStatus:(item.userId?item.picked?"ready_delivery":"picking":"unassigned");return product?{...withUploadedStock(product,uploaded),pickId:asText(item.id,item.productId),quantity:Math.max(1,asInt(item.quantity,1)),picked:item.picked,workflowStatus,available:item.available!==false,customerName:asText(item.customerName),customerPhone:normalizePhone(item.customerPhone),note:asText(item.note),assignedBy:asText(item.assignedBy),assigneeId:asText(item.userId),assigneeName:asText(assignee?.name,"Chưa phân công"),deliveryAssigneeId:asText(item.deliveryAssigneeId),deliveryAssigneeName:asText(deliveryAssignee?.name,"Chưa phân công"),deliveryTimeSlot:asText(item.deliveryTimeSlot),orderDate:normalizeOrderDate(item.orderDate,item.createdAt||Date.now()),createdAt:asInt(item.createdAt,Date.now())}:null;};
...
app.get("/api/orders/export.xlsx",async(req,res,next)=>{
  try {
    ...
    const headers=["Ngày đơn","Tên khách hàng","Số điện thoại","Khung giờ giao","Nhân viên giao hàng","SKU","Barcode","Tên sản phẩm","Số lượng","Đã lấy","Trạng thái","Nhân viên soạn","Người gán","Line POG","Mặt","Vị trí POG","Ghi chú","Ngày hoàn tất"];
    const rows=[headers];
    const append=(item,completed)=>{
      ...
      const deliveryAssignee=accountsById.get(item.deliveryAssigneeId);
      rows.push([date,asText(item.customerName,"Chưa đặt tên khách"),normalizePhone(item.customerPhone),asText(item.deliveryTimeSlot,"Chưa chọn"),asText(deliveryAssignee?.name,"Chưa phân công"),product.sku,barcode,product.name,Math.max(1,asInt(item.quantity,1)),item.picked?"Có":"Chưa",""+ (completed?"Hoàn tất":"Đang soạn"),asText(assignee?.name,"Chưa phân công"),asText(item.assignedBy),product.line?"Line "+product.line+String(product.side||""):"Chưa gán POG",product.side||"",product.bay||"",asText(item.note),completed?localDateKey(item.completedAt||Date.now()):""]);
    };
