...
type CustomerMaster = {id:string;phone:string;name:string;status?:string;vatExport?:string;memberCard?:string;group?:string;email?:string;taxId?:string;vatAddress?:string;deliveryAddress?:string;createdAt?:number;updatedAt?:number};
...
export default function Home() {
  ...
  const importCustomerMaster=async(file?:File)=>{if(!file)return;setBusy(true);try{const form=new FormData();form.set("file",file);const response=await fetch("/api/customers/import",{method:"POST",body:form}),payload=await response.json() as {error?:string;created?:number;updated?:number};if(!response.ok)throw new Error(payload.error||"Không thể nhập Master Data khách hàng");await loadData(true);setToast(`Đã nhập Master khách hàng · ${payload.created||0} mới · ${payload.updated||0} cập nhật`);}catch(cause){setToast(cause instanceof Error?cause.message:"Không thể nhập Master Data khách hàng");}finally{setBusy(false);if(customerExcelRef.current)customerExcelRef.current.value="";}};
...
function DailyReportView({reports,customers,month,onMonth,onSave,onImport,onExport,onExportCustomers,canManage,busy}:{reports:DailyReport[];customers:CustomerMaster[];month:string;onMonth:(value:string)=>void;onSave:(report:Record<string,unknown>)=>Promise<void>;onImport:()=>void;onExport:(month:string)=>void;onExportCustomers:()=>void;canManage:boolean;busy:boolean}) {
  ...
  const [form,setForm]=useState<Record<string,string>>(blank);const [masterView,setMasterView]=useState(false);const [saving,setSaving]=useState(false);
  const setField=(key:string,value:string)=>{if(key!=="phone"){setForm((current)=>({...current,[key]:value}));return;}const phone=value.replace(/\D/g,"");const customer=phone.length>=8?customers.find((item)=>item.phone.replace(/\D/g,"")===phone):undefined;setForm((current)=>customer?({...current,phone:value,customerName:customer.name||current.customerName,customerStatus:customer.status||current.customerStatus,vatExport:customer.vatExport||current.vatExport,memberCard:customer.memberCard||current.memberCard,customerGroup:customer.group||current.customerGroup,email:customer.email||current.email,taxId:customer.taxId||current.taxId,vatAddress:customer.vatAddress||current.vatAddress,deliveryAddress:customer.deliveryAddress||current.deliveryAddress}):({...current,phone:value}));};
