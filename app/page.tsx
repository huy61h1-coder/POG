      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {(["PRODUCTS","CHECK_STOCK"] as Tab[]).includes(tab)&&<ProductPager page={productPage} pages={totalPages} total={productResult.total} busy={productsLoading} onPage={setProductPage}/>}
           {tab==="ORDER"&&<OrderView items={data.picking.map(withPogLocation)} assignedItems={data.assignedPicking.map(withPogLocation)} history={(data.orderHistory||[]).map(withPogLocation)} users={data.users.filter((user)=>user.active)} canReassign={canManage(data.actor.role)} onReassignOrder={(pickIds,assigneeId)=>mutate("reassignOrder",{pickIds,assigneeId})} onExportHistory={exportOrderHistory} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})} onViewProduct={setProductDetails}/>}
...
function OrderView({items,assignedItems,history,users,canReassign,onReassignOrder,onExportHistory,onToggle,onViewProduct}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];users:UserRole[];canReassign:boolean;onReassignOrder:(pickIds:string[],assigneeId:string)=>Promise<boolean|undefined>;onExportHistory:(month:string)=>void;onToggle:(p:PickItem)=>void;onViewProduct:(product:Product)=>void}) {
  const [tab,setTab]=useState<"unpicked"|"working"|"completed"|"delivered">("working"),[orderDate,setOrderDate]=useState("");
...
function AssignedOrdersView({items,history,users,canReassign,onReassignOrder,assignedTitle="ĐƠN ĐÃ GIAO THEO KHÁCH HÀNG",historyTitle="LỊCH SỬ ĐƠN HÀNG",onExportHistory,onToggle,editablePickIds,onViewProduct,hideHistoryDate}:{items:AssignedPickItem[];history:OrderHistoryItem[];users?:UserRole[];canReassign?:boolean;onReassignOrder?:(pickIds:string[],assigneeId:string)=>Promise<boolean|undefined>;assignedTitle?:string;historyTitle?:string;onExportHistory:(month:string)=>void;onToggle?:(item:PickItem)=>void;editablePickIds?:Set<string>;onViewProduct?:(product:Product)=>void;hideHistoryDate?:boolean}) {
  ...
  const staffUsers=users||[],allowReassign=canReassign===true;
  const handleReassign=async(assigneeId:string)=>{if(!contextGroup||reassigning||!onReassignOrder)return;setReassigning(true);const ok=await onReassignOrder(contextGroup.orders.map((item)=>item.pickId),assigneeId);setReassigning(false);if(ok)setContextGroup(null);};
  ...
  return <>
    ...
    {selectedGroup&&selectedFirst&&<div className="modal-backdrop order-detail-backdrop"><section className="order-detail-modal" role="dialog" aria-modal="true" aria-label="Chi tiết đơn hàng"><div className="modal-head"><div><p>{selectedGroup.historyMode?"LỊCH SỬ ĐƠN HÀNG":assignedTitle}</p><h2>{selectedFirst.customerName||"Chưa đặt tên khách"}</h2></div><button type="button" onClick={()=>setSelectedGroup(null)} aria-label="Đóng">×</button></div><div className="order-detail-meta"><span><small>Số điện thoại</small><b>{selectedFirst.customerPhone||"—"}</b></span><span><small>Ngày đơn</small><b>{orderDateLabel(selectedDate)}</b></span><span><small>Tổng mặt hàng</small><b>{selectedGroup.orders.reduce((sum,item)=>sum+item.quantity,0)}</b></span><span><small>Trạng thái</small><b>{selectedGroup.historyMode?"Đã hoàn tất":`${selectedGroup.orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0)}/${selectedGroup.orders.reduce((sum,item)=>sum+item.quantity,0)} đã lấy`}</b></span></div><div className="order-detail-items"><div className="order-detail-items-head"><b>Danh sách hàng trong đơn</b><span>{selectedGroup.orders.length} dòng sản phẩm</span></div>{selectedGroup.orders.map((item,index)=>{const imageUrl=productImageUrl(item.imageUrl);return <article key={item.pickId}><em>{index+1}</em>{onViewProduct?<button type="button" className="order-product-image" onClick={()=>onViewProduct(item)} aria-label={`Xem chi tiết ${item.name}`}>{imageUrl?<img src={imageUrl} loading="lazy" decoding="async" alt=""/>:<span>Chưa có ảnh</span>}</button>:null}<div><button type="button" className="order-product-name" onClick={()=>onViewProduct?.(item)}><b>{item.name}</b></button><small>SKU {item.sku} · SL {item.quantity}{item.barcode||item.supplierBarcode?` · Barcode ${item.barcode||item.supplierBarcode}`:""}</small><small>{selectedGroup.historyMode?`Hoàn tất bởi: ${(item as OrderHistoryItem).completedBy||"—"}`:`Nhân viên: ${(item as AssignedPickItem).assigneeName||"—"}`}{item.note?` · ${item.note}`:""}</small></div><aside><strong>{item.shelfLine?`POG ${item.shelfLine}${item.shelfSide} · ${item.shelfPosition}`:"Chưa gán POG"}</strong>{!selectedGroup.historyMode&&onToggle&&editablePickIds?.has(item.pickId)&&<button type="button" className={item.picked?"order-picked-toggle done":"order-picked-toggle"} onClick={()=>onToggle(item)}>{item.picked?"✓ Đã lấy":"Đánh dấu đã lấy"}</button>}</aside></article>;})}</div><div className="modal-actions"><button type="button" className="primary" onClick={()=>setSelectedGroup(null)}>Đóng chi tiết</button></div></section></div>}
    {contextGroup&&<div className="order-reassign-menu" style={{left:menuLeft,top:menuTop}} role="menu" tabIndex={-1} aria-label="Gán đơn cho nhân viên" onClick={(event)=>event.stopPropagation()} onKeyDown={(event)=>event.stopPropagation()}><b>Gán đơn cho nhân viên</b>{staffUsers.length?staffUsers.map((user)=><button type="button" role="menuitem" key={user.userId} disabled={reassigning} onClick={()=>void handleReassign(user.userId)}>{user.name}<small>@{user.username}</small></button>):<span>Chưa có nhân viên hoạt động</span>}</div>}
  </>;
}
