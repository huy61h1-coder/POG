...
type Product = { id:string; sku:string; name:string; division:string; divisionName:string; department:string; departmentName:string; supplierBarcode:string; barcode:string; line:string; lineName:string; side:"A"|"B"; bay:number; price:number; stock:number; stockKnown?:boolean; loss:number; expDate:string; updatedAt?:number };
...
type StoreData = { actor:Actor; products:Product[]; productTotal:number; productStats:ProductStats; alertProducts:Product[]; availableLines:string[]; logs:Audit[]; picking:PickItem[]; users:UserRole[]; pogFiles:PogFile[]; lineConfigs?:LineConfig[]; manualChecks:{stock:Product[];loss:Product[];expiry:Product[]}; stockImport?:{fileName:string;updatedAt:number;recordCount:number;skipped:number}|null };
    ...
    <main className="ops-shell">
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {tab==="CHECK_STOCK"&&<StockCheckView products={products} total={productResult.total} role={data.actor.role} metadata={data.stockImport} job={stockImportJob} onImport={()=>stockExcelRef.current?.click()}/>}
...
function StockCheckView({products,total,role,metadata,job,onImport}:{products:Product[];total:number;role:Role;metadata?:StoreData["stockImport"];job:StockImportJob|null;onImport:()=>void}) {
  const active=Boolean(job&&["uploading","queued","processing"].includes(job.status));
  return <div><PageHead eyebrow="TỒN KHO TỪ FILE UPLOAD" title="Check Stock" subtitle="Chỉ hiển thị số tồn có trong file Stock đã upload; không lấy từ Master Data." actions={canManage(role)?<button className="primary" disabled={active} onClick={onImport}>{active?"Đang xử lý…":"↑ Upload Stock .xlsx"}</button>:undefined}/>
    <div className="master-help"><b>Định dạng file Stock</b><span>Cần 2 cột: SKU và STOCK (hoặc TỒN KHO). Upload mới sẽ thay thế toàn bộ danh sách tồn cũ · tối đa 500.000 dòng.</span></div>
    ...
    <div className="table-wrap"><table className="master-table"><thead><tr><th>SKU</th><th>TÊN SẢN PHẨM</th><th>Line</th><th>VỊ TRÍ</th><th>TỒN KHO</th></tr></thead><tbody>{products.map((p)=><tr key={p.id}><td><b>{p.sku}</b></td><td><strong>{p.name}</strong></td><td>{p.lineName?"Line "+p.line+" · "+p.lineName:"Chưa khớp Master Data"}</td><td>{p.bay?"Mặt "+p.side+" · Kệ "+p.bay:"—"}</td><td><StockBadge stock={p.stock}/></td></tr>)}{!products.length&&<tr><td colSpan={5}><div className="empty big"><b>Chưa có file Stock</b><span>Upload file Stock để bắt đầu xem tồn kho tách biệt.</span></div></td></tr>}</tbody></table></div><p className="stock-total">{money.format(total)} SKU có trong file Stock</p></div>;
}
