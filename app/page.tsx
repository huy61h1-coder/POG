...
export default function Home() {
  ...
  const pogLocationFor=(product:Pick<Product,"sku"|"barcode"|"supplierBarcode">)=>pogLocationIndex.get(normalize(product.sku))||pogLocationIndex.get(normalize(product.supplierBarcode||product.barcode));
  ...
  useEffect(()=>{
    ...
    const controller=new AbortController(),timer=window.setTimeout(async()=>{
      const params=new URLSearchParams({pageSize:"200"});if(activePogShelf){if(activePogSkus)params.set("skus",activePogSkus);}else{params.set("line",pogLine);params.set("side",pogSide);}if(pogSearch.trim())params.set("q",pogSearch.trim());
    ...
    return()=>{window.clearTimeout(timer);controller.abort();};
  },[actorUserId,pogLine,pogSide,pogSearch,productRefresh,activePogUpdated,activePogShelf,activePogSkus]);
  useEffect(()=>{
  ...
  const openProductOnMap = (product:Product) => { const location=pogLocationFor(product);setPogModal({line:location?.line||product.line,side:location?.side||product.side,selectedId:product.id});setPogSearch(product.sku);setQuery("");setProductPage(1); };
    ...
    <main className="ops-shell">
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {tab==="MAP"&&
            <MapView products={products} matchedLines={productResult.matchedLines||[]} pogFiles={data.pogFiles} lineConfigs={data.lineConfigs||[]} query={query} canManage={canManage(data.actor.role)} pogPdfCount={data.pogFiles.filter((file)=>file.mimeType==="application/pdf").length} onReanalyzeAll={()=>void reanalyzeAllPogs()} onOpen={(line,side)=>setPogModal({line,side})} onEdit={(lineConfig)=>setLineModal(lineConfig)}/>
          }
          ...
          {tab==="ORDER"&&<OrderView items={data.picking} assignedItems={data.assignedPicking} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})} onAvailability={(p)=>void mutate("markPickAvailability",{pickId:p.pickId,available:!p.available})} onQuantity={(p,quantity)=>void mutate("updatePickQuantity",{pickId:p.pickId,quantity})} onRemove={(p)=>void mutate("removePick",{pickId:p.pickId})} onClear={()=>void mutate("clearPick")} onMap={openProductOnMap}/>}
...
function MapView({products,matchedLines,pogFiles,lineConfigs,query,canManage,pogPdfCount,onReanalyzeAll,onOpen,onEdit}:{products:Product[];matchedLines:string[];pogFiles:PogFile[];lineConfigs:LineConfig[];query:string;canManage:boolean;pogPdfCount:number;onReanalyzeAll:()=>void;onOpen:(line:string,side:"A"|"B")=>void;onEdit:(lineConfig:LineConfig)=>void}) {
  ...
  const matched=new Set(pogMatchedLines.length?pogMatchedLines:matchedLines.length?matchedLines:products.filter((p)=>!query||normalize([p.name,p.sku,p.barcode,p.line,p.side,configByLine.get(p.line)?.name||aisleNames[p.line]].join(" ")).includes(needle)).map((p)=>p.line));
...
function AssignedOrdersView({items}:{items:AssignedPickItem[]}) {
  ...
  return <section className="assigned-orders"><div className="panel-title"><h2>ĐƠN ĐÃ GIAO THEO KHÁCH HÀNG</h2><span>{grouped.size} khách · {items.length} mặt hàng</span></div>{[...grouped.entries()].map(([key,orders])=>{const totalQuantity=orders.reduce((sum,item)=>sum+item.quantity,0);const pickedQuantity=orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0);return <article key={key}><header><b>Khách: {orders[0].customerName||"Chưa đặt tên"}</b><span>{totalQuantity} mặt hàng · {pickedQuantity}/{totalQuantity} đã lấy</span></header>{orders.map((item)=><div key={item.pickId}><em className={item.picked?"done":""}>{item.picked?"✓":"•"}</em><p><b>{item.name}</b><small>Nhân viên: {item.assigneeName||"—"} · SL {item.quantity} · SKU {item.sku}{item.note?" · Note: "+item.note:""}</small></p><strong>Line {item.line}{item.side}</strong></div>)}</article>})}{!items.length&&<div className="empty"><b>Chưa có đơn nào được giao.</b></div>}</section>;
}
