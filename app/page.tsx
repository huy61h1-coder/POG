  ...
  return (
    <main className="ops-shell">
      ...
      <header className="ops-topbar">
        <button className="ops-brand" aria-label="AEON Fulfillment SmartOps" onClick={()=>{setTab("DASHBOARD");setProductPage(1);}}><img style={{"--brand-logo-width":(data.appBrand?.logoSize||220)+"px","--brand-logo-width-mobile":Math.max(72,Math.round((data.appBrand?.logoSize||220)*.55))+"px"} as React.CSSProperties} src={data.appBrand?.logo||"/aeon-logo.svg"} alt="AEON Fulfillment SmartOps"/></button>
...
function SettingsModal({actor,users,theme,appLogo,logoSize,onLogo,onLogoSize,onTheme,onCreate,onUpdate,onPassword,onLogout,onClose}:{actor:Actor;users:UserRole[];theme:string;appLogo:string;logoSize:number;onLogo:(logo:string)=>Promise<boolean>;onLogoSize:(size:number)=>Promise<boolean>;onTheme:(v:string)=>void;onCreate:(account:{name:string;username:string;password:string;role:Role})=>Promise<boolean>;onUpdate:(account:{userId:string;name?:string;role?:Role;active?:boolean;password?:string})=>Promise<boolean>;onPassword:(currentPassword:string,newPassword:string)=>Promise<boolean>;onLogout:()=>Promise<void>;onClose:()=>void}) {
  ...
  const setLogoSize=(next:number)=>void onLogoSize(Math.max(120,Math.min(320,Math.round(next/10)*10)));
  ...
  return <div className="modal-backdrop"><section className="settings-modal account-settings"><div className="modal-head"><div><p>TÀI KHOẢN</p><h2>Tài khoản & phân quyền</h2></div><button onClick={onClose}>×</button></div>{actor.role==="ADMIN"&&<><h3>Logo ứng dụng</h3><div className="app-logo-setting"><img style={{width:Math.min(320,logoSize)+"px"}} src={appLogo} alt="Logo hiện tại"/><label className="ghost">Chọn logo<input type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" onChange={(event)=>updateLogo(event.target.files?.[0])}/></label><button className="ghost" onClick={()=>void onLogo("/aeon-logo.svg")}>Khôi phục AEON</button><div className="logo-size-control"><span>Kích thước logo</span><div className="logo-size-stepper"><button type="button" aria-label="Giảm kích thước logo" disabled={logoSize<=120} onClick={()=>setLogoSize(logoSize-10)}>−</button><b>{logoSize}px</b><button type="button" aria-label="Tăng kích thước logo" disabled={logoSize>=320} onClick={()=>setLogoSize(logoSize+10)}>+</button></div><input type="range" min="120" max="320" step="10" value={logoSize} aria-label="Kích thước logo" onChange={(event)=>setLogoSize(Number(event.target.value))}/></div><small>Logo được lưu chung trên hệ thống, áp dụng cho mọi tài khoản.</small></div></>}<h3>Giao diện</h3><div className="theme-row">{themes.map(([color,label])=><button key={color} className={theme===color?"active":""} data-color={color} onClick={()=>onTheme(color)}><i/>{label}</button>)}</div>
