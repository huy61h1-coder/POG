      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {(["PRODUCTS","CHECK_STOCK"] as Tab[]).includes(tab)&&<ProductPager page={productPage} pages={totalPages} total={productResult.total} busy={productsLoading} onPage={setProductPage}/>}
           {tab==="ORDER"&&<OrderView items={data.picking.map(withPogLocation)} assignedItems={data.assignedPicking.map(withPogLocation)} history={(data.orderHistory||[]).map(withPogLocation)} onExportHistory={exportOrderHistory} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})}/>}
...
function OrderView({items,assignedItems,history,onExportHistory,onToggle}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];onExportHistory:(month:string)=>void;onToggle:(p:PickItem)=>void}) {
  const [tab,setTab]=useState<"unpicked"|"working"|"completed"|"delivered">("working");
  const orderGroups=groupOrderItems(assignedItems),flatten=(groups:AssignedPickItem[][])=>groups.flat();
  ...
  const unpickedItems=flatten(unpickedGroups),workingItems=flatten(workingGroups),completedItems=flatten(completedGroups),deliveredOrderCount=new Set(history.map(orderGroupKey)).size;
  ...
  return <div className={`order-tab-shell order-tab-${tab}`}><nav className="order-tabs" aria-label="Trạng thái đơn hàng"><button type="button" className={tab==="unpicked"?"active":""} onClick={()=>setTab("unpicked")} aria-pressed={tab==="unpicked"}><span>Chưa soạn</span><b>{unpickedGroups.length}</b></button><button type="button" className={tab==="working"?"active":""} onClick={()=>setTab("working")} aria-pressed={tab==="working"}><span>Đang soạn</span><b>{workingGroups.length}</b></button><button type="button" className={tab==="completed"?"active":""} onClick={()=>setTab("completed")} aria-pressed={tab==="completed"}><span>Đã soạn xong</span><b>{completedGroups.length}</b></button><button type="button" className={tab==="delivered"?"active":""} onClick={()=>setTab("delivered")} aria-pressed={tab==="delivered"}><span>Đã giao</span><b>{deliveredOrderCount}</b></button></nav><div className="order-tab-content"><AssignedOrdersView items={panels.items} history={panels.history} assignedTitle={panelTitle} historyTitle="ĐƠN ĐÃ GIAO" onExportHistory={onExportHistory} onToggle={tab==="working"?onToggle:undefined} editablePickIds={editablePickIds}/></div></div>;
}
function AssignedOrdersView({items,history,assignedTitle="ĐƠN ĐÃ GIAO THEO KHÁCH HÀNG",historyTitle="LỊCH SỬ ĐƠN HÀNG",onExportHistory,onToggle,editablePickIds}:{items:AssignedPickItem[];history:OrderHistoryItem[];assignedTitle?:string;historyTitle?:string;onExportHistory:(month:string)=>void;onToggle?:(item:PickItem)=>void;editablePickIds?:Set<string>}) {
