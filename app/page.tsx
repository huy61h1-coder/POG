...
function ProductsView({products,total,role,importResult,importJob,onAdd,onEdit,onDelete,onMap,onPick,onExport,onImport}:{products:Product[];total:number;role:Role;importResult:MasterImportResult|null;importJob:MasterImportJob|null;onAdd:()=>void;onEdit:(p:Product)=>void;onDelete:(p:Product)=>void;onMap:(p:Product)=>void;onPick:(p:Product)=>void;onExport:()=>void;onImport:()=>void}) {
  ...
  return <div><PageHead eyebrow="MASTER DATA" title="Master Data sản phẩm" subtitle={money.format(total)+" SKU phù hợp"} actions={<><button className="ghost" onClick={onExport}>↓ Xuất Master CSV</button>{canManage(role)&&<button className="ghost" disabled={importing} onClick={onImport}>{importing?"Đang nhập Excel…":"↑ Nhập Excel (.xlsx)"}</button>}{canManage(role)&&<button className="primary" onClick={onAdd}>+ Thêm sản phẩm</button>}</>}/>
    <div className="master-help"><b>Định dạng nhập Excel</b><span>Giữ đúng 9 cột bắt buộc và thêm IMAGE URL / LINK HÌNH ẢNH nếu có · đọc sheet đầu tiên · tối đa 500.000 dòng / 100 MB · cập nhật theo SKU. Sản phẩm không có link ảnh sẽ hiện trạng thái “Chưa có ảnh” để bổ sung thủ công.</span></div>
    ...
    <div className="table-wrap"><table className="master-table"><thead><tr><th>ẢNH</th><th>SKU</th><th>TÊN SẢN PHẨM</th><th>Division</th><th>DIVISION NAME</th><th>Department</th><th>DEPARTMENT</th><th>SUPPLIER BARCODE</th><th>VỊ TRÍ KỆ POG</th><th/></tr></thead><tbody>{products.map((p)=><tr key={p.id}>
...
function ProductModal({value,onChange,onClose,onSave}:{value:Product;onChange:(p:Product)=>void;onClose:()=>void;onSave:()=>void}) {
  ...
  return <div className="modal-backdrop"><section className="form-modal product-modal"><div className="modal-head"><div><p>MASTER DATA</p><h2>{value.id?"Chỉnh sửa sản phẩm":"Thêm sản phẩm"}</h2></div><button onClick={onClose}>×</button></div><div className="form-grid">
    ...
    <label>Giá<input type="number" min="0" value={value.price} onChange={(e)=>set("price",Number(e.target.value))}/></label><label className="wide">Link hình ảnh<input value={value.imageUrl||""} onChange={(e)=>set("imageUrl",e.target.value)} placeholder="https://.../san-pham.jpg hoặc /uploads/..."/></label>
