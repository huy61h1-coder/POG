...
type Product = { id:string; sku:string; name:string; division:string; divisionName:string; department:string; departmentName:string; supplierBarcode:string; barcode:string; imageUrl?:string; line:string; lineName:string; side:"A"|"B"; bay:number; shelfLine?:string; shelfSide?:string; shelfPosition?:number; price:number; stock:number; systemStock?:number; sales?:number; stockKnown?:boolean; loss:number; expDate:string; manualStock?:number; manualLoss?:number; inboundDate?:string; withdrawDate?:string; updatedAt?:number };
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {(["PRODUCTS","CHECK_STOCK"] as Tab[]).includes(tab)&&<ProductPager page={productPage} pages={totalPages} total={productResult.total} busy={productsLoading} onPage={setProductPage}/>}
           {tab==="ORDER"&&<OrderView items={data.picking.map(withPogLocation)} assignedItems={data.assignedPicking.map(withPogLocation)} history={(data.orderHistory||[]).map(withPogLocation)} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})} onAvailability={(p)=>void mutate("markPickAvailability",{pickId:p.pickId,available:!p.available})} onQuantity={(p,quantity)=>void mutate("updatePickQuantity",{pickId:p.pickId,quantity})} onRemove={(p)=>void mutate("removePick",{pickId:p.pickId})} onClear={()=>void mutate("clearPick")} onMap={openProductOnMap}/>}
...
function ProductsView({products,total,role,importResult,importJob,onAdd,onEdit,onDelete,onMap,onPick,onExport,onImport}:{products:Product[];total:number;role:Role;importResult:MasterImportResult|null;importJob:MasterImportJob|null;onAdd:()=>void;onEdit:(p:Product)=>void;onDelete:(p:Product)=>void;onMap:(p:Product)=>void;onPick:(p:Product)=>void;onExport:()=>void;onImport:()=>void}) {
  ...
  return <div><PageHead eyebrow="MASTER DATA" title="Master Data sản phẩm" subtitle={money.format(total)+" SKU phù hợp"} actions={<><button className="ghost" onClick={onExport}>↓ Xuất Master CSV</button>{canManage(role)&&<button className="ghost" disabled={importing} onClick={onImport}>{importing?"Đang nhập Excel…":"↑ Nhập Excel (.xlsx)"}</button>}{canManage(role)&&<button className="primary" onClick={onAdd}>+ Thêm sản phẩm</button>}</>}/>
    <div className="master-help"><b>Định dạng nhập Excel</b><span>Giữ đúng 9 cột nguồn · đọc sheet đầu tiên · tối đa 500.000 dòng / 100 MB · cập nhật theo SKU. Cột Line / Line Name trong Excel chỉ là metadata nguồn, không phải vị trí kệ; vị trí kệ chỉ được gán khi SKU hoặc barcode khớp POG.</span></div>
    ...
    <div className="table-wrap"><table className="master-table"><thead><tr><th>SKU</th><th>TÊN SẢN PHẨM</th><th>Division</th><th>DIVISION NAME</th><th>Department</th><th>DEPARTMENT</th><th>SUPPLIER BARCODE</th><th>VỊ TRÍ KỆ POG</th><th/></tr></thead><tbody>{products.map((p)=><tr key={p.id}>
      ...
      <td data-label="VỊ TRÍ KỆ POG"><b>{p.shelfLine?`Line ${p.shelfLine}${p.shelfSide} · Vị trí ${p.shelfPosition}`:"Chưa gán POG"}</b></td>
      <td data-label=""><div className="row-actions"><button onClick={()=>onPick(p)}>+ Đơn</button><button disabled={!p.shelfLine} onClick={()=>onMap(p)}>Vị trí</button>{canManage(role)&&<button onClick={()=>onEdit(p)}>Sửa</button>}{canManage(role)&&<button className="danger-text" onClick={()=>{if(window.confirm("Xóa sản phẩm “"+p.name+"”?"))onDelete(p)}}>Xóa</button>}</div></td>
    </tr>)}{!products.length&&<tr><td colSpan={9}><div className="empty big"><b>Chưa có sản phẩm phù hợp</b><span>Thử đổi bộ lọc hoặc nhập Master Data từ Excel.</span></div></td></tr>}</tbody></table></div></div>;
}
function StockCheckView({products,total,role,metadata,job,onImport,onExport,onAssign}:{products:Product[];total:number;role:Role;metadata?:StoreData["stockImport"];job:StockImportJob|null;onImport:()=>void;onExport:()=>void;onAssign:(product:Product)=>void}) {
  ...
  return <div><PageHead eyebrow="TỒN KHO TỪ FILE UPLOAD" title="Check Stock" subtitle="Chỉ hiển thị số tồn có trong file Stock đã upload; không lấy từ Master Data." actions={<>{products.length>0&&<button onClick={onExport}>↓ Xuất Stock</button>}{canManage(role)&&<button className="primary" disabled={active} onClick={onImport}>{active?"Đang xử lý…":"↑ Upload Stock .xlsx"}</button>}</>}/>
    <div className="master-help"><b>Định dạng file Stock</b><span>SKU · TÊN SẢN PHẨM · Department · Department Name · Division · Line · Line Name · Sales · Closing Stock. Khi nhập, hệ thống dùng SKU, Sales và Closing Stock; Line / Line Name trong file là metadata nguồn và không dùng để xác định kệ. Vị trí kệ chỉ lấy từ POG đã liên kết.</span></div>
    ...
    <div className="table-wrap"><table className="master-table"><thead><tr><th>SKU</th><th>TÊN SẢN PHẨM</th><th>Department</th><th>Division</th><th>VỊ TRÍ KỆ POG</th><th>Sales</th><th>Closing Stock</th>{canManage(role)&&<th/>}</tr></thead><tbody>{products.map((p)=><tr key={p.id}><td><b>{p.sku}</b></td><td><strong>{p.name}</strong></td><td>{p.departmentName||p.department||"—"}</td><td>{p.division||"—"}</td><td>{p.shelfLine?`Line ${p.shelfLine}${p.shelfSide} · Vị trí ${p.shelfPosition}`:"Chưa gán POG"}</td><td>{money.format(p.sales||0)}</td><td><StockBadge stock={p.stock}/></td>{canManage(role)&&<td><button className="primary assign-button" disabled={p.stock<=0} onClick={()=>onAssign(p)}>Gán đơn</button></td>}</tr>)}{!products.length&&<tr><td colSpan={canManage(role)?8:7}><div className="empty big"><b>Chưa có file Stock</b><span>Upload file Stock để bắt đầu xem tồn kho tách biệt.</span></div></td></tr>}</tbody></table></div><p className="stock-total">{money.format(total)} SKU có trong file Stock</p></div>;
}
...
function OrderView({items,assignedItems,history,onToggle,onAvailability,onQuantity,onRemove,onClear,onMap}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];onToggle:(p:PickItem)=>void;onAvailability:(p:PickItem)=>void;onQuantity:(p:PickItem,quantity:number)=>void;onRemove:(p:PickItem)=>void;onClear:()=>void;onMap:(p:PickItem)=>void}) {
  ...
  const sorted=[...items].sort((a,b)=>Number(Boolean(a.picked))-Number(Boolean(b.picked))||shelfRoute(a)-shelfRoute(b)||(a.shelfSide||"Z").localeCompare(b.shelfSide||"Z")||(a.shelfPosition||0)-(b.shelfPosition||0));
...
function ProductModal({value,onChange,onClose,onSave}:{value:Product;onChange:(p:Product)=>void;onClose:()=>void;onSave:()=>void}) {
  ...
  return <div className="modal-backdrop"><section className="form-modal"><div className="modal-head"><div><p>MASTER DATA</p><h2>{value.id?"Chỉnh sửa sản phẩm":"Thêm sản phẩm"}</h2></div><button onClick={onClose}>×</button></div><div className="form-grid">
    ...
    <label>Giá<input type="number" min="0" value={value.price} onChange={(e)=>set("price",Number(e.target.value))}/></label><p className="form-note wide">Vị trí kệ không nhập ở Master Data: hệ thống chỉ gán Line/Mặt/vị trí sau khi SKU hoặc barcode khớp với file POG. Tồn kho lấy từ tab Check Stock.</p>
