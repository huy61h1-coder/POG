...
function MapView({products,query,onOpen}:{products:Product[];query:string;onOpen:(line:string,side:"A"|"B")=>void}) {
  const matched=new Set(products.filter((p)=>!query||normalize(p.name+" "+p.sku+" "+p.barcode).includes(normalize(query))).map((p)=>p.line));
  return <div><PageHead eyebrow="BẢN ĐỒ CỬA HÀNG" title="Sơ đồ POG" subtitle="Chọn dãy và mặt kệ để xem vị trí sản phẩm chi tiết."/>
    <div className="full-map"><div className="map-top">DAILY · FRESH FOOD</div><div className="map-aisles">{Object.entries(aisleNames).map(([line,name])=><div key={line} className={matched.has(line)&&query?"match":""}><button onClick={()=>onOpen(line,"A")}><small>{name}</small><strong>{line}</strong><span>Mặt A</span></button><button onClick={()=>onOpen(line,"B")}><small>{name}</small><strong>{line}</strong><span>Mặt B</span></button></div>)}</div><div className="promo">PROMOTION</div><div className="you-are">● BẠN Ở ĐÂY</div></div></div>;
}
