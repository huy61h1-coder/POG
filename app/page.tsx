...
type StoreData = { actor:Actor; products:Product[]; productTotal:number; productStats:ProductStats; alertProducts:Product[]; availableLines:string[]; logs:Audit[]; picking:PickItem[]; users:UserRole[]; pogFiles:PogFile[]; lineConfigs?:LineConfig[] };
  ...
  useEffect(()=>{
    ...
    const controller=new AbortController(),timer=window.setTimeout(async()=>{
      ...
      try {
        ...
        const response=await fetch("/api/products?"+params,{cache:"no-store",signal:controller.signal}),payload=await response.json() as ProductPage&{error?:string};
    ...
    <main className="ops-shell">
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {tab==="STOCK"&&<CheckGrid kind="stock" products={products} onAdjust={(p,d)=>void mutate("adjustStock",{id:p.id,delta:d})}/>}
          {tab==="LOSS"&&<CheckGrid kind="loss" products={products} onAdjust={(p,d)=>void mutate("adjustLoss",{id:p.id,delta:d})}/>}
          {tab==="DATE"&&<DateGrid products={products} onChange={(p,value)=>void mutate("updateDate",{id:p.id,expDate:value})}/>}
          {(["PRODUCTS","STOCK","LOSS","DATE"] as Tab[]).includes(tab)&&<ProductPager page={productPage} pages={totalPages} total={productResult.total} busy={productsLoading} onPage={setProductPage}/>}
