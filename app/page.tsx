...
export default function Home() {
  ...
  return (
    <main className="ops-shell">
      ...
      {settingsOpen&&<SettingsModal actor={data.actor} users={data.users} theme={theme} appLogo={data.appBrand?.logo||"/aeon-logo.svg"} logoSize={data.appBrand?.logoSize||220} onLogo={(logo)=>mutate("updateAppBrand",{logo})} onLogoSize={(logoSize)=>mutate("updateAppBrand",{logoSize})} onTheme={setTheme} onCreate={(account)=>mutate("createAccount",{account})} onUpdate={(account)=>mutate("updateAccount",{account})} onPassword={(currentPassword,newPassword)=>mutate("changeOwnPassword",{currentPassword,newPassword})} onLogout={logout} onClose={()=>setSettingsOpen(false)}/>} 
...
function SettingsModal({actor,users,theme,appLogo,logoSize,onLogo,onLogoSize,onTheme,onCreate,onUpdate,onPassword,onLogout,onClose}:{actor:Actor;users:UserRole[];theme:string;appLogo:string;logoSize:number;onLogo:(logo:string)=>Promise<boolean>;onLogoSize:(size:number)=>Promise<boolean>;onTheme:(v:string)=>void;onCreate:(account:{name:string;username:string;password:string;role:Role})=>Promise<boolean>;onUpdate:(account:{userId:string;name?:string;role?:Role;active?:boolean;password?:string})=>Promise<boolean>;onPassword:(currentPassword:string,newPassword:string)=>Promise<boolean>;onLogout:()=>Promise<void>;onClose:()=>void}) {
  ...
  const updateLogo=(file?:File)=>{if(!file)return;if(file.size>1024*1024){window.alert("Logo tối đa 1 MB.");return;}const reader=new FileReader();reader.onload=()=>void onLogo(String(reader.result));reader.readAsDataURL(file);};
  return <div className="modal-backdrop"><section className="settings-modal account-settings"><div className="modal-head"><div><p>TÀI KHOẢN</p><h2>Tài khoản & phân quyền</h2></div><button onClick={onClose}>×</button></div>{actor.role==="ADMIN"&&<><h3>Logo ứng dụng</h3><div className="app-logo-setting"><img style={{width:Math.min(220,logoSize)+"px"}} src={appLogo} alt="Logo hiện tại"/><label className="ghost">Chọn logo<input type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" onChange={(event)=>updateLogo(event.target.files?.[0])}/></label><button className="ghost" onClick={()=>void onLogo("/aeon-logo.svg")}>Khôi phục AEON</button><label className="logo-size-control">Kích thước: <b>{logoSize}px</b><input type="range" min="120" max="320" step="10" value={logoSize} onChange={(event)=>void onLogoSize(Number(event.target.value))}/></label><small>Logo được lưu chung trên hệ thống, áp dụng cho mọi tài khoản.</small></div></>}<h3>Giao diện</h3><div className="theme-row">{themes.map(([color,label])=><button key={color} className={theme===color?"active":""} data-color={color} onClick={()=>onTheme(color)}><i/>{label}</button>)}</div>
...
function PogModal({modal,setModal,products,total,file,search,setSearch,canUpload,uploadRef,onUpload,onAppend,onReanalyze,onPageChange,onPick,onClose}:{modal:{line:string;side:"A"|"B";selectedId?:string};setModal:(v:{line:string;side:"A"|"B";selectedId?:string})=>void;products:Product[];total:number;file?:PogFile;search:string;setSearch:(v:string)=>void;canUpload:boolean;uploadRef:React.RefObject<HTMLInputElement|null>;onUpload:(f?:File)=>void;onAppend:(f?:File)=>void;onReanalyze:()=>void;onPageChange:(page:number)=>void;onPick:(p:Product)=>void;onClose:()=>void}) {
  ...
  const [pdfPage,setPdfPage]=useState(file?.page||1),appendRef=useRef<HTMLInputElement>(null),fileCount=file?.sources?.length||1;
  ...
  return <div className="modal-backdrop pog-backdrop"><section className="pog-modal"><div className="pog-head"><div><p>SƠ ĐỒ KỆ CHI TIẾT</p><h2>Line {modal.line} · {aisleNames[modal.line]||"Khu vực"}</h2></div><div className="side-switch"><button className={modal.side==="A"?"active":""} onClick={()=>switchSide("A")}>Mặt A</button><button className={modal.side==="B"?"active":""} onClick={()=>switchSide("B")}>Mặt B</button></div>{file?.mimeType==="application/pdf"&&!file.shelfImage&&<label className="pdf-page-picker">Trang sơ đồ<select value={pdfPage} onChange={(e)=>{const page=Number(e.target.value);setPdfPage(page);onPageChange(page)}}>{Array.from({length:12},(_,index)=><option key={index+1} value={index+1}>Trang {index+1}{index<2?" · thường dùng":""}</option>)}</select></label>}{file?.shelfImage&&<span className="pog-linked-count">Đã ghép {fileCount} file · {file.positions?.length||0} vị trí</span>}{canUpload&&file?.mimeType==="application/pdf"&&<button className="upload-pog reanalyze-pog" title="Tạo lại ảnh ghép từ tất cả PDF đang lưu" onClick={onReanalyze}>↻ Ghép lại sát viền</button>}{canUpload&&file&&<button className="upload-pog add-pog" title="Giữ POG hiện có và ghép thêm một PDF" onClick={()=>appendRef.current?.click()}>＋ Thêm file POG</button>}{canUpload&&<button className="upload-pog" title="Thay toàn bộ POG bằng PDF hoặc ảnh mới" onClick={()=>uploadRef.current?.click()}>↑ {file?"Thay POG":"Tải POG PDF/ảnh"}</button>}<button className="close-pog" onClick={onClose}>×</button></div>
    <div className="pog-body pog-canvas-layout"><aside className="pog-list"><label>⌕<input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Tìm SKU, barcode, tên…"/><b>{products.length}/{total} SP</b></label>{total>products.length&&<p className="pog-limit-note">Đang hiện 200 kết quả đầu · nhập SKU hoặc tên để tìm chính xác.</p>}<div>{products.map((p)=><button key={p.id} className={p.id===modal.selectedId?"active":""} onClick={()=>setModal({...modal,selectedId:p.id})}><span>{file?.shelfImage?`POG ${file.line}${file.side}`:`Kệ ${p.bay}`}</span><div><small>SKU {p.sku}</small><b>{p.name}</b><em>{p.supplierBarcode||p.barcode}</em></div><StockBadge stock={p.stock}/></button>)}{!products.length&&<div className="empty big">Chưa có sản phẩm liên kết trong Master Data.</div>}</div>{selected&&<section className="pog-selected"><div><b>{file?.shelfImage?`POG Line ${file.line}${file.side}`:`Line ${selected.line}${selected.side} · Kệ ${selected.bay}`}</b><span>{selected.name}</span><small>{file?.shelfImage?(linkedPositions.length?`${linkedPositions.length} vị trí POG: ${linkedPositions.map((position)=>position.number).join(", ")}`:"Chưa tìm thấy STT trên ảnh POG")+" · ":""}Tồn {selected.stock} · Loss {selected.loss} · HSD {selected.expDate||"chưa có"}</small></div><button disabled={selected.stock===0} onClick={()=>onPick(selected)}>+ Thêm vào đơn</button></section>}</aside><div className="pog-visual">{file?(file.shelfImage?<PogShelfImage file={file} selected={selected} positions={linkedPositions}/>:file.mimeType==="application/pdf"?<iframe src={"/api/pog?id="+file.id+"#page="+pdfPage} title={"POG PDF trang "+pdfPage}/>:<img src={"/api/pog?id="+file.id} alt={"POG Line "+modal.line}/>):<div className="pog-empty-state"><span>POG {modal.line}{modal.side}</span><h3>Chưa có file POG cho mặt kệ này</h3><p>Tải PDF để tự crop, ghép nhiều file, liên kết SKU với stock và khoanh vị trí sản phẩm theo cùng một quy trình chuẩn.</p>{canUpload&&<button onClick={()=>uploadRef.current?.click()}>↑ Tải POG PDF</button>}</div>} {file&&file.mimeType==="application/pdf"&&!file.shelfImage&&<div className="pog-page-badge">Đã lưu mặt kệ · Trang {pdfPage}</div>}{selected&&!file?.shelfImage&&<div className="image-marker" style={{left:`${Math.min(91,8+selected.bay*10)}%`}}><i/><span>Kệ {selected.bay}<b>{selected.name}</b></span></div>}<div className="pog-file-label">{file?file.fileName:`POG Line ${modal.line}${modal.side}`}</div></div></div>
