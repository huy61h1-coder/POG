...
type PickItem = Product & { pickId:string; quantity:number; picked:boolean|number; workflowStatus?:WorkflowStatus; available?:boolean; customerName:string; customerPhone?:string; note:string; assignedBy:string; assigneeId?:string; assigneeName?:string; deliveryAssigneeId?:string; deliveryAssigneeName?:string; deliveryTimeSlot?:string; orderDate?:string; createdAt?:number };
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {(["PRODUCTS","CHECK_STOCK"] as Tab[]).includes(tab)&&<ProductPager page={productPage} pages={totalPages} total={productResult.total} busy={productsLoading} onPage={setProductPage}/>}
           {tab==="ORDER"&&<OrderView items={data.picking.map(withPogLocation)} assignedItems={data.assignedPicking.map(withPogLocation)} history={(data.orderHistory||[]).map(withPogLocation)} actor={data.actor} users={data.users.filter((user)=>user.active)} canReassign={canManage(data.actor.role)} onReassignOrder={(pickIds,assigneeId)=>mutate("reassignOrder",{pickIds,assigneeId})} onCompletePicking={(pickIds)=>mutate("completePickingOrder",{pickIds})} onAssignDelivery={(pickIds,deliveryAssigneeId)=>mutate("assignDeliveryOrder",{pickIds,deliveryAssigneeId})} onCompleteDelivery={(pickIds)=>mutate("completeDeliveryOrder",{pickIds})} onReopenDelivered={(pickIds)=>mutate("reopenDeliveredOrder",{pickIds})} onExportHistory={exportOrderHistory} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})} onViewProduct={setProductDetails}/>}
...
function OrderView({items,assignedItems,history,actor,users,canReassign,onReassignOrder,onCompletePicking,onAssignDelivery,onCompleteDelivery,onReopenDelivered,onExportHistory,onToggle,onViewProduct}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];actor:Actor;users:UserRole[];canReassign:boolean;onReassignOrder:(pickIds:string[],assigneeId:string)=>Promise<boolean|undefined>;onCompletePicking:(pickIds:string[])=>Promise<boolean|undefined>;onAssignDelivery:(pickIds:string[],assigneeId:string)=>Promise<boolean|undefined>;onCompleteDelivery:(pickIds:string[])=>Promise<boolean|undefined>;onReopenDelivered:(pickIds:string[])=>Promise<boolean|undefined>;onExportHistory:(month:string)=>void;onToggle:(p:PickItem)=>void;onViewProduct:(product:Product)=>void}) {
  ...
  const unpickedGroups=orderGroups.filter((group)=>group.every((item)=>item.workflowStatus==="unassigned")),workingGroups=orderGroups.filter((group)=>group.every((item)=>item.workflowStatus==="picking")),completedGroups=orderGroups.filter((group)=>group.every((item)=>item.workflowStatus==="ready_delivery"));
...
function AssignedOrdersView({items,history,actor,users,canReassign,onReassignOrder,onCompletePicking,onAssignDelivery,onCompleteDelivery,onReopenDelivered,assignedTitle="ĐƠN ĐÃ GIAO THEO KHÁCH HÀNG",historyTitle="LỊCH SỬ ĐƠN HÀNG",onExportHistory,onToggle,editablePickIds,onViewProduct,hideHistoryDate}:{items:AssignedPickItem[];history:OrderHistoryItem[];actor?:Actor;users?:UserRole[];canReassign?:boolean;onReassignOrder?:(pickIds:string[],assigneeId:string)=>Promise<boolean|undefined>;onCompletePicking?:(pickIds:string[])=>Promise<boolean|undefined>;onAssignDelivery?:(pickIds:string[],assigneeId:string)=>Promise<boolean|undefined>;onCompleteDelivery?:(pickIds:string[])=>Promise<boolean|undefined>;onReopenDelivered?:(pickIds:string[])=>Promise<boolean|undefined>;assignedTitle?:string;historyTitle?:string;onExportHistory:(month:string)=>void;onToggle?:(item:PickItem)=>void;editablePickIds?:Set<string>;onViewProduct?:(product:Product)=>void;hideHistoryDate?:boolean}) {
  ...
  const renderGroup=(key:string,orders:PickItem[],historyMode=false)=>{const totalQuantity=orders.reduce((sum,item)=>sum+item.quantity,0),pickedQuantity=orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0),first=orders[0],date=orderDateKey(first.orderDate||((first as OrderHistoryItem).completedAt)),groupId=(historyMode?"history|":"assigned|")+key,statusLabel=historyMode?"Đã giao":first.workflowStatus==="unassigned"?"Chờ phân công":first.workflowStatus==="ready_delivery"?"Chờ gán tài xế":pickedQuantity+"/"+totalQuantity+" đã lấy";return <article key={groupId} className="customer-order-group" onContextMenu={(event)=>{const kind=historyMode?"reopen":first.workflowStatus==="ready_delivery"?"delivery":"picker";if((kind==="reopen"&&actor?.role==="ADMIN")||(kind==="delivery"&&canReassign)||(kind==="picker"&&canReassign)){event.preventDefault();setContextGroup({orders,x:event.clientX,y:event.clientY,kind});}}}><button type="button" className="customer-order-header" onClick={()=>setSelectedGroup({key,orders,historyMode})} aria-label={`Xem chi tiết đơn của ${first.customerName||"khách hàng"}`}><span><b>Khách: {first.customerName||"Chưa đặt tên"}</b>{first.customerPhone&&<small>☎ {first.customerPhone}</small>}<small>{orderDateLabel(date)}</small>{first.deliveryTimeSlot&&<small>🕘 {first.deliveryTimeSlot}</small>}{first.deliveryAssigneeName&&first.deliveryAssigneeName!=="Chưa phân công"&&<small>🚚 {first.deliveryAssigneeName}</small>}</span><strong>{totalQuantity} mặt hàng · {statusLabel}<i>›</i></strong></button></article>;};
  ...
  const selectedAllPicked=Boolean(selectedGroup?.orders.length&&selectedGroup.orders.every((item)=>Boolean(item.picked)));
  const canCompletePicking=Boolean(selectedGroup&&!selectedGroup.historyMode&&selectedFirst?.workflowStatus==="picking"&&selectedAllPicked&&onCompletePicking&&(actor?.role==="ADMIN"||selectedGroup.orders.every((item)=>item.assigneeId===actor?.userId)));
  const canCompleteDelivery=Boolean(selectedGroup&&!selectedGroup.historyMode&&selectedFirst?.workflowStatus==="ready_delivery"&&onCompleteDelivery&&(actor?.role==="ADMIN"||((actor?.workType==="DELIVERY"||actor?.workType==="BOTH")&&selectedGroup.orders.every((item)=>item.deliveryAssigneeId===actor?.userId))));
  ...
  return <>
    ...
    {selectedGroup&&selectedFirst&&<div className="modal-backdrop order-detail-backdrop"><section className="order-detail-modal" role="dialog" aria-modal="true" aria-label="Chi tiết đơn hàng"><div className="modal-head"><div><p>{selectedGroup.historyMode?"LỊCH SỬ ĐƠN HÀNG":assignedTitle}</p><h2>{selectedFirst.customerName||"Chưa đặt tên khách"}</h2></div><button type="button" onClick={()=>setSelectedGroup(null)} aria-label="Đóng">×</button></div><div className="order-detail-meta"><span><small>Số điện thoại</small><b>{selectedFirst.customerPhone||"—"}</b></span><span><small>Ngày đơn</small><b>{orderDateLabel(selectedDate)}</b></span><span><small>Tổng mặt hàng</small><b>{selectedGroup.orders.reduce((sum,item)=>sum+item.quantity,0)}</b></span><span><small>Trạng thái</small><b>{selectedGroup.historyMode?"Đã giao":selectedFirst.workflowStatus==="unassigned"?"Chờ phân công":selectedFirst.workflowStatus==="ready_delivery"?"Chờ gán tài xế":`${selectedGroup.orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0)}/${selectedGroup.orders.reduce((sum,item)=>sum+item.quantity,0)} đã lấy`}</b></span></div><div className="order-detail-items"><div className="order-detail-items-head"><b>Danh sách hàng trong đơn</b><span>{selectedGroup.orders.length} dòng sản phẩm</span></div>{selectedGroup.orders.map((item,index)=>{const imageUrl=productImageUrl(item.imageUrl);return <article key={item.pickId}><em>{index+1}</em>{onViewProduct?<button type="button" className="order-product-image" onClick={()=>onViewProduct(item)} aria-label={`Xem chi tiết ${item.name}`}>{imageUrl?<img src={imageUrl} loading="lazy" decoding="async" alt=""/>:<span>Chưa có ảnh</span>}</button>:null}<div><button type="button" className="order-product-name" onClick={()=>onViewProduct?.(item)}><b>{item.name}</b></button><small>SKU {item.sku} · SL {item.quantity}{item.barcode||item.supplierBarcode?` · Barcode ${item.barcode||item.supplierBarcode}`:""}</small><small>{selectedGroup.historyMode?`Hoàn tất bởi: ${(item as OrderHistoryItem).completedBy||"—"}`:`Nhân viên soạn: ${(item as AssignedPickItem).assigneeName||"Chưa phân công"}`}{item.deliveryAssigneeName?` · Giao hàng: ${item.deliveryAssigneeName}`:""}{item.deliveryTimeSlot?` · ${item.deliveryTimeSlot}`:""}{item.note?` · ${item.note}`:""}</small></div><aside><strong>{item.shelfLine?`POG ${item.shelfLine}${item.shelfSide} · ${item.shelfPosition}`:"Chưa gán POG"}</strong>{!selectedGroup.historyMode&&onToggle&&editablePickIds?.has(item.pickId)&&<button type="button" className={item.picked?"order-picked-toggle done":"order-picked-toggle"} onClick={()=>onToggle(item)}>{item.picked?"✓ Đã lấy":"Đánh dấu đã lấy"}</button>}</aside></article>;})}</div><div className="modal-actions">{canCompletePicking&&<button type="button" className="primary" disabled={reassigning} onClick={()=>void handleCompletePicking()}>Hoàn tất soạn hàng</button>}{canCompleteDelivery&&<button type="button" className="primary" disabled={reassigning} onClick={()=>void handleCompleteDelivery()}>Hoàn tất giao hàng</button>}{selectedGroup.historyMode&&actor?.role==="ADMIN"&&onReopenDelivered&&<button type="button" className="ghost" disabled={reassigning} onClick={()=>{setContextGroup({orders:selectedGroup.orders,x:window.innerWidth/2,y:window.innerHeight/2,kind:"reopen"});setSelectedGroup(null);}}>Mở lại chỉnh sửa</button>}<button type="button" className="primary" onClick={()=>setSelectedGroup(null)}>Đóng chi tiết</button></div></section></div>}
