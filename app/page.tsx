      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {(["PRODUCTS","CHECK_STOCK"] as Tab[]).includes(tab)&&<ProductPager page={productPage} pages={totalPages} total={productResult.total} busy={productsLoading} onPage={setProductPage}/>}
           {tab==="ORDER"&&<OrderView items={data.picking.map(withPogLocation)} assignedItems={data.assignedPicking.map(withPogLocation)} history={(data.orderHistory||[]).map(withPogLocation)} onExportHistory={exportOrderHistory} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})} onViewProduct={setProductDetails}/>}
...
function OrderView({items,assignedItems,history,onExportHistory,onToggle,onViewProduct}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];onExportHistory:(month:string)=>void;onToggle:(p:PickItem)=>void;onViewProduct:(product:Product)=>void}) {
