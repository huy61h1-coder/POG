...
type PickItem = Product & { pickId:string; quantity:number; picked:boolean|number; available?:boolean; customerName:string; customerPhone?:string; note:string; assignedBy:string; orderDate?:string; createdAt?:number };
...
type StoreData = { actor:Actor; products:Product[]; productTotal:number; productStats:ProductStats; alertProducts:Product[]; availableLines:string[]; logs:Audit[]; picking:PickItem[]; assignedPicking:AssignedPickItem[]; orderHistory?:OrderHistoryItem[]; users:UserRole[]; pogFiles:PogFile[]; lineConfigs?:LineConfig[]; appBrand?:{logo:string;logoSize:number;logoSizeDesktop?:number;logoSizeMobile?:number;updatedAt:number}; manualChecks:{checkLoss?:Product[];stock?:Product[];loss?:Product[];expiry:Product[]}; stockImport?:{fileName:string;updatedAt:number;recordCount:number;skipped:number}|null };
...
async function analyzePogFiles(files:File[]):Promise<PogAnalysis|null> {
  const analyses:PogAnalysis[]=[];for(const file of files){const analysis=await analyzePogPdf(file);if(analysis)analyses.push(analysis);}return combinePogAnalyses(analyses);
}
...
export default function Home() {
  ...
  const reanalyzePog = async () => { if(!pogModal||!activePog)return;setBusy(true);try{const files=await loadPogSourceFiles(activePog),analysis=await analyzePogFiles(files);if(!analysis)throw new Error("Không tìm thấy vùng kệ trong các PDF POG");await savePogAnalysis(files[0],pogModal.line,pogModal.side,analysis,"reanalyze");await loadData(true);setToast(`Đã ghép lại ${files.length} file · ${analysis.positions.length} vị trí sản phẩm`);}catch(cause){setToast(cause instanceof Error?cause.message:"Không thể ghép lại POG");}finally{setBusy(false);} };
    ...
    <main className="ops-shell">
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {tab==="DASHBOARD"&&<Dashboard products={data.alertProducts.map(withPogLocation)} totalProducts={data.productTotal} logs={data.logs} totals={data.productStats} onGo={(next,stock)=>{setTab(next);setStockFilter(stock||"all");setQuery("");setProductPage(1);}}/>}
          {tab==="MAP"&&
          ...
          {(["PRODUCTS","CHECK_STOCK"] as Tab[]).includes(tab)&&<ProductPager page={productPage} pages={totalPages} total={productResult.total} busy={productsLoading} onPage={setProductPage}/>}
           {tab==="ORDER"&&<OrderView items={data.picking.map(withPogLocation)} assignedItems={data.assignedPicking.map(withPogLocation)} history={(data.orderHistory||[]).map(withPogLocation)} onExportHistory={exportOrderHistory} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})} onAvailability={(p)=>void mutate("markPickAvailability",{pickId:p.pickId,available:!p.available})} onQuantity={(p,quantity)=>void mutate("updatePickQuantity",{pickId:p.pickId,quantity})} onRemove={(p)=>void mutate("removePick",{pickId:p.pickId})} onClear={()=>void mutate("clearPick")} onMap={openProductOnMap}/>}
...
function Dashboard({products,totalProducts,logs,totals,onGo}:{products:Product[];totalProducts:number;logs:Audit[];totals:ProductStats;onGo:(tab:Tab,stock?:"all"|"low"|"out")=>void}) {
...
function ProductInfoModal({product,onClose,onMap,onPick}:{product:Product;onClose:()=>void;onMap?:()=>void;onPick?:()=>Promise<void>}) {
  ...
  return <div className="modal-backdrop product-info-backdrop"><section className="form-modal product-info-modal" role="dialog" aria-modal="true" aria-label={`Thông tin sản phẩm ${product.name||product.sku}`}>
    ...
    <div className="product-info-body"><div className="product-info-hero"><div className="product-info-image">{product.imageUrl?<><img src={product.imageUrl} alt={product.name||"Ảnh sản phẩm"} onError={(event)=>{event.currentTarget.style.display="none";}}/><span className="product-info-image-fallback">Không tải được ảnh</span></>:<span>Chưa có ảnh sản phẩm</span>}</div><div className="product-info-title"><span>SKU {product.sku||"—"}</span><h3>{product.name||"Chưa có tên sản phẩm"}</h3><p>{product.supplierBarcode||product.barcode?`Barcode ${product.supplierBarcode||product.barcode}`:"Chưa có barcode"}</p><StockBadge stock={product.stock}/></div></div><ProductDetailGrid product={product} full/></div>
    <div className="modal-actions"><button className="ghost" onClick={onClose}>Đóng</button>{onMap&&<button className="ghost" onClick={onMap}>Xem vị trí POG</button>}{onPick&&<button className="primary" disabled={product.stock<=0||adding} onClick={()=>void add()}>{adding?"Đang thêm…":product.stock<=0?"Hết hàng":"+ Thêm vào đơn"}</button>}</div>
  </section></div>;
}
function ProductsView({products,total,role,importResult,importJob,onAdd,onEdit,onDelete,onView,onMap,onPick,onExport,onImport}:{products:Product[];total:number;role:Role;importResult:MasterImportResult|null;importJob:MasterImportJob|null;onAdd:()=>void;onEdit:(p:Product)=>void;onDelete:(p:Product)=>void;onView:(p:Product)=>void;onMap:(p:Product)=>void;onPick:(p:Product)=>void;onExport:()=>void;onImport:()=>void}) {
  ...
  return <div><PageHead eyebrow="MASTER DATA" title="Master Data sản phẩm" subtitle={money.format(total)+" SKU phù hợp"} actions={<><button className="ghost" onClick={onExport}>↓ Xuất Master (.xlsx)</button>{canManage(role)&&<button className="ghost" disabled={importing} onClick={onImport}>{importing?"Đang nhập Excel…":"↑ Nhập Excel (.xlsx)"}</button>}{canManage(role)&&<button className="primary" onClick={onAdd}>+ Thêm sản phẩm</button>}</>}/>
    ...
    <div className="table-wrap"><table className="master-table compact-table master-data-table"><thead><tr><th>ẢNH</th><th>SKU</th><th>TÊN SẢN PHẨM</th><th>Sale</th><th>Stock</th><th>Division</th><th>Department</th><th>VỊ TRÍ KỆ POG</th><th>THAO TÁC</th></tr></thead><tbody>{products.map((p)=>{const expanded=expandedRows.has(p.id);return <Fragment key={p.id}><tr><td data-label="ẢNH"><button type="button" className="product-table-image-button" onClick={()=>onView(p)} aria-label={`Xem thông tin ${p.name||p.sku}`}><div className="product-table-image">{p.imageUrl?<img src={p.imageUrl} alt=""/>:<span>Chưa có ảnh</span>}</div></button></td><td data-label="SKU"><button type="button" className="product-name-button" onClick={()=>onView(p)}>{p.sku}</button></td><td data-label="TÊN SẢN PHẨM"><button type="button" className="product-name-button product-name-button-wide" onClick={()=>onView(p)}><strong>{p.name}</strong></button></td><td data-label="Sale">{money.format(p.sales??0)}</td><td data-label="Stock"><StockBadge stock={p.stock}/></td><td data-label="Division">{p.division||"—"}</td><td data-label="Department">{p.department||"—"}</td><td data-label="VỊ TRÍ KỆ POG"><b>{p.shelfLine?`Line ${p.shelfLine}${p.shelfSide} · Vị trí ${p.shelfPosition}`:"Chưa gán POG"}</b></td><td data-label="THAO TÁC"><div className="row-actions compact-row-actions"><button type="button" className="details-button" aria-expanded={expanded} onClick={()=>toggleRow(p.id)}>{expanded?"Thu gọn":"Xem đầy đủ"}</button><button type="button" onClick={()=>onPick(p)}>+ Đơn</button><button type="button" disabled={!p.shelfLine} onClick={()=>onMap(p)}>Vị trí</button>{canManage(role)&&<button type="button" onClick={()=>onEdit(p)}>Sửa</button>}{canManage(role)&&<button type="button" className="danger-text" onClick={()=>{if(window.confirm("Xóa sản phẩm “"+p.name+"”?"))onDelete(p)}}>Xóa</button>}</div></td></tr>{expanded&&<tr className="master-detail-row"><td colSpan={9}><ProductDetailGrid product={p}/></td></tr>}</Fragment>;})}{!products.length&&<tr><td colSpan={9}><div className="empty big"><b>Chưa có sản phẩm phù hợp</b><span>Thử đổi bộ lọc hoặc nhập Master Data từ Excel.</span></div></td></tr>}</tbody></table></div></div>;
}
...
function OrderView({items,assignedItems,history,onExportHistory,onToggle,onAvailability,onQuantity,onRemove,onClear,onMap}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];onExportHistory:(month:string)=>void;onToggle:(p:PickItem)=>void;onAvailability:(p:PickItem)=>void;onQuantity:(p:PickItem,quantity:number)=>void;onRemove:(p:PickItem)=>void;onClear:()=>void;onMap:(p:PickItem)=>void}) {
  ...
  return <div className="order-page"><PageHead eyebrow="PICKING LIST" title="Đơn đang soạn" subtitle={pickedUnits+"/"+totalUnits+" sản phẩm đã lấy · sắp theo vị trí kệ POG"} actions={items.length?<button className="primary" onClick={finish}>{next?"Kết thúc sớm":"Hoàn tất đơn"}</button>:undefined}/><div className="order-top-panels"><AssignedOrdersView items={assignedItems} history={history} onExportHistory={onExportHistory}/></div><section className="order-summary" aria-label="Tóm tắt đơn soạn"><article><span>Đang soạn</span><b>{items.length}</b><small>{totalUnits} sản phẩm</small></article><article><span>Đã lấy</span><b>{pickedUnits}</b><small>trên {totalUnits} sản phẩm</small></article><article><span>Nhóm đã giao</span><b>{assignedCustomerCount}</b><small>khách hàng</small></article><article><span>Lịch sử</span><b>{history.length}</b><small>mặt hàng đã hoàn tất</small></article></section>{next&&<section className="next-pick"><div><small>ĐIỂM LẤY TIẾP THEO</small><b>{locationLabel(next)}</b><span>{next.name} · SL {next.quantity}</span></div><button disabled={!next.shelfLine} onClick={()=>onMap(next)}>Mở vị trí →</button></section>}<div className="order-progress-large"><i><span style={{width:percent+"%"}}/></i><b>{percent}%</b></div>
    <div className="order-list" aria-label="Danh sách sản phẩm đang soạn">{sorted.map((p,index)=>{const date=orderDateKey(p.orderDate||p.createdAt),previous=index?orderDateKey(sorted[index-1].orderDate||sorted[index-1].createdAt):"";return <Fragment key={p.pickId}>{date!==previous&&<div className="order-day-divider"><b>{orderDateLabel(date)}</b><span>{dateTotals.get(date)||0} mặt hàng</span></div>}<article className={p.picked?"picked":""}><div className="pick-product-thumb" title={p.departmentName||p.department||"Chưa có Department"}>{p.imageUrl?<img src={p.imageUrl} alt=""/>:<span>{(p.departmentName||p.department||"Chưa có Dept").slice(0,22)}</span>}</div><button className={"availability-toggle "+(p.available!==false?"available":"unavailable")} aria-pressed={p.available===false} aria-label={p.available!==false?"Chuyển sản phẩm sang hết hàng":"Chuyển sản phẩm sang còn hàng"} title={p.available!==false?"Nhấn để báo hết hàng":"Nhấn để báo còn hàng"} onClick={()=>onAvailability(p)}><i/><span><b>{p.available!==false?"Còn hàng":"Hết hàng"}</b><small>{p.available!==false?"Nhấn để báo hết":"Nhấn để hoàn tác"}</small></span></button><button className="pick-check" aria-label={p.picked?"Đánh dấu chưa lấy":"Đánh dấu đã lấy"} onClick={()=>onToggle(p)}>{p.picked?"✓":""}</button><div className="pick-product-info"><small>SKU {p.sku}{p.customerName?" · Khách: "+p.customerName:""}{p.customerPhone?" · "+p.customerPhone:""}</small><b>{p.name}</b><span>{p.barcode||p.supplierBarcode?"Barcode: "+(p.barcode||p.supplierBarcode)+" · ":""}{locationLabel(p)}{p.note?" · "+p.note:""}</span></div><div className="pick-quantity"><button disabled={p.quantity<=1} onClick={()=>onQuantity(p,p.quantity-1)}>−</button><b>{p.quantity}</b><button onClick={()=>onQuantity(p,p.quantity+1)}>+</button></div><StockBadge stock={p.stock}/><button disabled={!p.shelfLine} onClick={()=>onMap(p)}>Vị trí</button><button className="danger-text" onClick={()=>onRemove(p)}>Bỏ</button></article></Fragment>})}{!items.length&&<div className="empty big"><b>Đơn soạn đang trống</b><span>Chờ đơn được giao từ Check Stock hoặc thêm sản phẩm từ tìm kiếm.</span></div>}</div></div>;
}
function AssignedOrdersView({items,history,onExportHistory}:{items:AssignedPickItem[];history:OrderHistoryItem[];onExportHistory:(month:string)=>void}) {
  const [historyDate,setHistoryDate]=useState(""),[historyCustomer,setHistoryCustomer]=useState(""),[exportMonth,setExportMonth]=useState(defaultOrderMonth),[openGroups,setOpenGroups]=useState<Set<string>>(new Set());
  const groupItems=<T extends PickItem>(rows:T[])=>{const grouped=new Map<string,T[]>();for(const item of rows){const name=(item.customerName||"Chưa đặt tên khách").trim().replace(/\s+/g," "),phone=(item.customerPhone||"").trim(),date=orderDateKey(item.orderDate||item.createdAt),key=[date,normalize(name)||"chua dat ten khach",phone].join("|");grouped.set(key,[...(grouped.get(key)||[]),{...item,customerName:name}]);}return grouped;};
  const assignedGroups=groupItems(items),historyGroups=groupItems(history.filter((item)=>{const matchesDate=!historyDate||orderDateKey(item.orderDate||item.completedAt)===historyDate,needle=normalize(historyCustomer),matchesCustomer=!needle||normalize(`${item.customerName||""} ${item.customerPhone||""}`).includes(needle);return matchesDate&&matchesCustomer;}));
  const toggle=(key:string)=>setOpenGroups((current)=>{const next=new Set(current);if(next.has(key))next.delete(key);else next.add(key);return next;});
  const renderGroup=(key:string,orders:PickItem[],historyMode=false)=>{const totalQuantity=orders.reduce((sum,item)=>sum+item.quantity,0),pickedQuantity=orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0),first=orders[0],date=orderDateKey(first.orderDate||((first as OrderHistoryItem).completedAt));return <article key={(historyMode?"history|":"assigned|")+key} className="customer-order-group"><button type="button" className="customer-order-header" onClick={()=>toggle((historyMode?"history|":"assigned|")+key)} aria-expanded={openGroups.has((historyMode?"history|":"assigned|")+key)}><span><b>Khách: {first.customerName||"Chưa đặt tên"}</b>{first.customerPhone&&<small>☎ {first.customerPhone}</small>}<small>{orderDateLabel(date)}</small></span><strong>{totalQuantity} mặt hàng · {historyMode?"Đã hoàn tất":pickedQuantity+"/"+totalQuantity+" đã lấy"} <i>{openGroups.has((historyMode?"history|":"assigned|")+key)?"⌃":"⌄"}</i></strong></button>{openGroups.has((historyMode?"history|":"assigned|")+key)&&<div className="customer-order-items">{orders.map((item)=><div key={item.pickId}><em className={item.picked?"done":""}>{item.picked?"✓":"•"}</em><p><b>{item.name}</b><small>{historyMode?`Hoàn tất bởi: ${(item as OrderHistoryItem).completedBy||"—"}`:`Nhân viên: ${(item as AssignedPickItem).assigneeName||"—"}`} · SL {item.quantity} · SKU {item.sku}{item.note?" · Note: "+item.note:""}</small></p><strong>{item.shelfLine?`POG ${item.shelfLine}${item.shelfSide} · ${item.shelfPosition}`:"Chưa gán POG"}</strong></div>)}</div>}</article>;};
  return <><section className="assigned-orders"><div className="panel-title"><h2>ĐƠN ĐÃ GIAO THEO KHÁCH HÀNG</h2><span>{assignedGroups.size} khách/ngày · {items.length} mặt hàng</span></div>{[...assignedGroups.entries()].map(([key,orders])=>renderGroup(key,orders))}{!items.length&&<div className="empty"><b>Chưa có đơn nào được giao.</b><span>Đơn mới sẽ được nhóm theo tên và số điện thoại khách hàng.</span></div>}</section><section className="order-history-panel"><div className="panel-title"><h2>LỊCH SỬ ĐƠN HÀNG</h2><span>{historyGroups.size} khách/ngày · {history.length} mặt hàng</span></div><div className="order-history-toolbar"><label>Tìm khách hàng<input type="search" value={historyCustomer} onChange={(event)=>setHistoryCustomer(event.target.value)} placeholder="Tên hoặc số điện thoại"/></label><label>Chọn ngày xem lại<input type="date" value={historyDate} onChange={(event)=>setHistoryDate(event.target.value)}/></label><label>Tháng lưu Excel<input type="month" value={exportMonth} onChange={(event)=>setExportMonth(event.target.value)}/></label><button type="button" className="primary" disabled={!exportMonth} onClick={()=>onExportHistory(exportMonth)}>↓ Xuất Excel</button><button type="button" className="ghost" disabled={!historyDate&&!historyCustomer} onClick={()=>{setHistoryDate("");setHistoryCustomer("")}}>Tất cả</button></div>{historyGroups.size?[...historyGroups.entries()].map(([key,orders])=>renderGroup(key,orders,true)):<div className="empty"><b>{historyDate||historyCustomer?"Không có đơn phù hợp":"Chưa có lịch sử đơn hàng"}</b><span>Khi bấm Hoàn tất đơn, hệ thống sẽ lưu lại ngày, khách hàng và danh sách hàng.</span></div>}</section></>;
}
...
function ProductModal({value,onChange,onClose,onSave}:{value:Product;onChange:(p:Product)=>void;onClose:()=>void;onSave:()=>void}) {
  ...
  return <div className="modal-backdrop"><section className="form-modal product-modal"><div className="modal-head"><div><p>MASTER DATA</p><h2>{value.id?"Chỉnh sửa sản phẩm":"Thêm sản phẩm"}</h2></div><button onClick={onClose}>×</button></div><div className="form-grid">
    ...
    <label>Giá bán retail<input type="number" min="0" value={value.price} onChange={(e)=>set("price",Number(e.target.value))}/></label><label>Giá khuyến mãi<input type="number" min="0" value={value.promoPrice||0} onChange={(e)=>set("promoPrice",Number(e.target.value))}/></label><label>Sale<input type="number" min="0" value={value.sales||0} onChange={(e)=>set("sales",Number(e.target.value))}/></label><label className="wide">Link hình ảnh<input value={value.imageUrl||""} onChange={(e)=>set("imageUrl",e.target.value)} placeholder="https://.../san-pham.jpg hoặc /uploads/..."/></label>
    <div className="product-image-tools wide"><button type="button" className="ghost" onClick={()=>imageRef.current?.click()}>↑ Chọn ảnh từ máy</button>{value.imageUrl&&<button type="button" className="ghost" onClick={()=>onChange({...value,imageUrl:""})}>Xóa ảnh</button>}<input ref={imageRef} hidden type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/svg+xml" onChange={(e)=>{selectImage(e.target.files?.[0]);if(imageRef.current)imageRef.current.value="";}}/>{value.imageUrl?<img src={value.imageUrl} alt={value.name||"Ảnh sản phẩm"}/>:<span>Chưa có ảnh · thêm link hoặc chọn tệp để hiển thị sản phẩm</span>}</div>
    <p className="form-note wide">Cột IMAGE URL / LINK HÌNH ẢNH trong Excel được đọc tự động. Khi sửa thủ công, có thể dán link ảnh hoặc chọn tệp PNG, JPG, WEBP, GIF, SVG (tối đa 1 MB).</p>
...
function PogModal({modal,setModal,products,total,file,search,setSearch,canUpload,uploadRef,onUpload,onAppend,onReanalyze,onPageChange,onView,onPick,onClose}:{modal:{line:string;side:"A"|"B";selectedId?:string};setModal:(v:{line:string;side:"A"|"B";selectedId?:string})=>void;products:Product[];total:number;file?:PogFile;search:string;setSearch:(v:string)=>void;canUpload:boolean;uploadRef:React.RefObject<HTMLInputElement|null>;onUpload:(f?:File)=>void;onAppend:(f?:File)=>void;onReanalyze:()=>void;onPageChange:(page:number)=>void;onView:(p:Product)=>void;onPick:(p:Product)=>void;onClose:()=>void}) {
  ...
  const linkedPositions=selected?file?.positions?.filter((position)=>[position.sku,position.barcode].map(normalize).some((key)=>selectedKeys.has(key)))||[]:[];
  const positionsFor=(product:Product)=>{const keys=new Set([product.sku,product.barcode,product.supplierBarcode].map(normalize).filter(Boolean));return file?.positions?.filter((position)=>[position.sku,position.barcode].map(normalize).some((key)=>keys.has(key)))||[];};
  const mobileProducts=search.trim()?products.slice(0,12):[];
  const [pdfPage,setPdfPage]=useState(file?.page||1),[searchExpanded,setSearchExpanded]=useState(false),appendRef=useRef<HTMLInputElement>(null),fileCount=file?.sources?.length||1;
  const switchSide=(side:"A"|"B")=>{setSearch("");setPdfPage(1);setModal({line:modal.line,side})};
  return <div className="modal-backdrop pog-backdrop"><section className="pog-modal"><div className="pog-head"><div><p>SƠ ĐỒ KỆ CHI TIẾT</p><h2>Line {modal.line} · {aisleNames[modal.line]||"Khu vực"}</h2></div><div className="side-switch"><button className={modal.side==="A"?"active":""} onClick={()=>switchSide("A")}>Mặt A</button><button className={modal.side==="B"?"active":""} onClick={()=>switchSide("B")}>Mặt B</button></div>{file?.mimeType==="application/pdf"&&!file.shelfImage&&<label className="pdf-page-picker">Trang sơ đồ<select value={pdfPage} onChange={(e)=>{const page=Number(e.target.value);setPdfPage(page);onPageChange(page)}}>{Array.from({length:12},(_,index)=><option key={index+1} value={index+1}>Trang {index+1}{index<2?" · thường dùng":""}</option>)}</select></label>}{file?.shelfImage&&<span className="pog-linked-count">Đã ghép {fileCount} file · {file.positions?.length||0} vị trí</span>}{canUpload&&file?.mimeType==="application/pdf"&&<button className="upload-pog reanalyze-pog" title="Tạo lại ảnh ghép từ tất cả PDF đang lưu" onClick={onReanalyze}>↻ Ghép lại sát viền</button>}{canUpload&&file&&<button className="upload-pog add-pog" title="Giữ POG hiện có và ghép thêm một PDF" onClick={()=>appendRef.current?.click()}>＋ Thêm file POG</button>}{canUpload&&<button className="upload-pog" title="Thay toàn bộ POG bằng PDF hoặc ảnh mới" onClick={()=>uploadRef.current?.click()}>↑ {file?"Thay POG":"Tải POG PDF/ảnh"}</button>}<button className="close-pog" onClick={onClose}>×</button></div>
    <div className={`pog-body pog-canvas-layout${searchExpanded?" pog-search-expanded":""}${selected?" pog-has-product":""}`}>
      <aside className="pog-list"><label>⌕<input value={search} onChange={(e)=>setSearch(e.target.value)} onFocus={()=>setSearchExpanded(true)} onBlur={()=>window.setTimeout(()=>setSearchExpanded(false),160)} placeholder="Tìm SKU, barcode, tên…"/><b>{products.length}/{total} SP</b></label>{total>products.length&&<p className="pog-limit-note">Đang hiện 200 kết quả đầu · nhập SKU hoặc tên để tìm chính xác.</p>}<div>{products.map((p)=><button key={p.id} className={p.id===selected?.id?"active":""} onClick={()=>{setModal({...modal,selectedId:p.id});onView(p)}}><span>{file?.shelfImage?`POG ${file.line}${file.side}`:"POG chưa chuẩn"}</span><div><small>SKU {p.sku}</small><b>{p.name}</b><em>{p.supplierBarcode||p.barcode}</em></div><StockBadge stock={p.stock}/></button>)}{!products.length&&<div className="empty big">{file?.shelfImage?"Chưa có sản phẩm liên kết trong Master Data.":"POG cần được phân tích và ghép trước khi liên kết sản phẩm."}</div>}</div>{selected&&<section className="pog-selected"><div><b>{file?.shelfImage?`POG Line ${file.line}${file.side}`:"Chưa chuẩn hóa POG"}</b><span>{selected.name}</span><small>{file?.shelfImage?(linkedPositions.length?`${linkedPositions.length} vị trí POG: ${linkedPositions.map((position)=>position.number).join(", ")}`:"Chưa tìm thấy STT trên ảnh POG")+" · ":""}Tồn {selected.stock} · Loss {selected.loss} · HSD {selected.expDate||"chưa có"}</small></div><button disabled={selected.stock===0} onClick={()=>onPick(selected)}>+ Thêm vào đơn</button></section>}</aside>
    ...
    <input ref={uploadRef} hidden type="file" accept=".pdf,application/pdf,image/*" onChange={(e)=>{onUpload(e.target.files?.[0]);e.currentTarget.value="";}}/><input ref={appendRef} hidden type="file" accept=".pdf,application/pdf" onChange={(e)=>{onAppend(e.target.files?.[0]);e.currentTarget.value="";}}/></section></div>;
}
