...
type PickItem = Product & { pickId:string; quantity:number; picked:boolean|number; available?:boolean; customerName:string; customerPhone?:string; note:string; assignedBy:string; assigneeId?:string; assigneeName?:string; deliveryAssigneeId?:string; deliveryAssigneeName?:string; deliveryTimeSlot?:string; orderDate?:string; createdAt?:number };
...
function StockCheckView({products,total,role,metadata,job,onImport,onExport,onView,onAssign}:{products:Product[];total:number;role:Role;metadata?:StoreData["stockImport"];job:StockImportJob|null;onImport:()=>void;onExport:()=>void;onView:(p:Product)=>void;onAssign:(product:Product)=>void}) {
  ...
  return <div><PageHead eyebrow="TỒN KHO TỪ FILE UPLOAD" title="Check Stock" subtitle="Chỉ hiển thị số tồn có trong file Stock đã upload; không lấy từ Master Data." actions={<>{products.length>0&&<button onClick={onExport}>↓ Xuất Stock (.xlsx)</button>}{canManage(role)&&<button className="primary" disabled={active} onClick={onImport}>{active?"Đang xử lý…":"↑ Upload Stock .xlsx"}</button>}</>}/>
	...
	function AssignPickModal({product,users,customerNames,onClose,onAssign}:{product:Product;users:UserRole[];customerNames:string[];onClose:()=>void;onAssign:(assignment:{assigneeId?:string;deliveryAssigneeId?:string;deliveryTimeSlot:string;quantity:number;customerName:string;customerPhone:string;note:string;orderDate:string})=>Promise<void>}) {
	  const [assigneeId,setAssigneeId]=useState(""),[deliveryAssigneeId,setDeliveryAssigneeId]=useState(""),[deliveryTimeSlot,setDeliveryTimeSlot]=useState(""),[quantity,setQuantity]=useState(1),[customerName,setCustomerName]=useState(""),[customerPhone,setCustomerPhone]=useState(""),[orderDate,setOrderDate]=useState(()=>orderDateKey(Date.now())),[note,setNote]=useState("");
          const pickerUsers=users.filter((user)=>user.workType!="DELIVERY"),deliveryUsers=users.filter((user)=>user.workType==="DELIVERY"||user.workType==="BOTH");
	  const phoneDigits=customerPhone.replace(/\D/g,"");
	  const valid=Boolean(customerName.trim()&&phoneDigits.length>=8&&quantity<=product.stock);
	  return <div className="modal-backdrop"><section className="form-modal"><div className="modal-head"><div><p>GÁN SOẠN ĐƠN TỪ STOCK</p><h2>Giao sản phẩm cho đơn hàng</h2></div><button onClick={onClose}>×</button></div><div className="form-grid"><p className="form-note wide"><b>{product.name}</b><br/>SKU {product.sku} · Tồn hiện có: {money.format(product.stock)}</p><label className="wide">Tên nhân viên soạn hàng<select value={assigneeId} onChange={(event)=>setAssigneeId(event.target.value)}><option value="">Chưa phân công (gán sau trong Đơn soạn)</option>{pickerUsers.map((user)=><option key={user.userId} value={user.userId}>{user.name} · @{user.username}</option>)}</select></label><label>Tên khách hàng<input list="assigned-customer-names" value={customerName} onChange={(event)=>setCustomerName(event.target.value)} placeholder="Chọn hoặc nhập tên khách"/><datalist id="assigned-customer-names">{customerNames.map((name)=><option key={name} value={name}/>)}</datalist></label><label>Số điện thoại khách<input type="tel" inputMode="tel" autoComplete="tel" value={customerPhone} onChange={(event)=>setCustomerPhone(event.target.value)} placeholder="Ví dụ: 0901234567"/></label><label>Ngày giao hàng<input type="date" min={todayOrderDate} value={orderDate} onChange={(event)=>setOrderDate(event.target.value)}/></label><label>Khung giờ giao hàng<select value={deliveryTimeSlot} onChange={(event)=>setDeliveryTimeSlot(event.target.value)}><option value="">Chưa chọn khung giờ</option><option>08:00 - 10:00</option><option>10:00 - 12:00</option><option>12:00 - 14:00</option><option>14:00 - 16:00</option><option>16:00 - 18:00</option><option>18:00 - 20:00</option></select></label><label className="wide">Nhân viên giao hàng<select value={deliveryAssigneeId} onChange={(event)=>setDeliveryAssigneeId(event.target.value)}><option value="">Chưa phân công</option>{deliveryUsers.map((user)=><option key={user.userId} value={user.userId}>{user.name} · @{user.username}</option>)}</select>{!deliveryUsers.length&&<small className="assign-user-empty">Chưa có nhân viên có quyền Giao hàng. Cấu hình trong Cài đặt → Phân quyền.</small>}</label><label>Số lượng<input type="number" min="1" max={product.stock} value={quantity} onChange={(event)=>setQuantity(Math.max(1,Number(event.target.value)||1))}/></label><label className="wide">Ghi chú Stock / đơn hàng<textarea rows={3} value={note} onChange={(event)=>setNote(event.target.value)} placeholder="Ví dụ: Ưu tiên hàng hạn dùng xa, giao buổi chiều…"/></label></div><p className="form-note order-assignment-hint">Có thể để trống nhân viên soạn/giao và phân công sau trong mục Đơn soạn.</p><div className="modal-actions"><button className="ghost" onClick={onClose}>Hủy</button><button className="primary" disabled={!valid||!orderDate} onClick={()=>void onAssign({assigneeId:assigneeId||undefined,deliveryAssigneeId:deliveryAssigneeId||undefined,deliveryTimeSlot,quantity,customerName:customerName.trim(),customerPhone:phoneDigits,orderDate,note:note.trim()})}>Tạo đơn</button></div></section></div>;
}
