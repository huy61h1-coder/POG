...
function OrderView({items,assignedItems,history,onExportHistory,onToggle,onAvailability,onQuantity,onRemove,onClear,onMap}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];onExportHistory:(month:string)=>void;onToggle:(p:PickItem)=>void;onAvailability:(p:PickItem)=>void;onQuantity:(p:PickItem,quantity:number)=>void;onRemove:(p:PickItem)=>void;onClear:()=>void;onMap:(p:PickItem)=>void}) {
  const [tab,setTab]=useState<"working"|"assigned"|"unpicked"|"history">("working");
  const unpickedItems=assignedItems.filter((item)=>!item.picked);
  const panels=tab==="history"?{items:[],history}:{items:tab==="unpicked"?unpickedItems:assignedItems,history:[]};
  return <div className={`order-tab-shell order-tab-${tab}`}><nav className="order-tabs" aria-label="Trạng thái đơn hàng"><button type="button" className={tab==="working"?"active":""} onClick={()=>setTab("working")} aria-pressed={tab==="working"}><span>Đang soạn</span><b>{items.length}</b></button><button type="button" className={tab==="assigned"?"active":""} onClick={()=>setTab("assigned")} aria-pressed={tab==="assigned"}><span>Đã giao</span><b>{assignedItems.length}</b></button><button type="button" className={tab==="unpicked"?"active":""} onClick={()=>setTab("unpicked")} aria-pressed={tab==="unpicked"}><span>Chưa soạn</span><b>{unpickedItems.length}</b></button><button type="button" className={tab==="history"?"active":""} onClick={()=>setTab("history")} aria-pressed={tab==="history"}><span>Lịch sử hoàn tất</span><b>{history.length}</b></button></nav>{tab==="working"?<OrderViewLegacy items={items} assignedItems={assignedItems} history={history} onExportHistory={onExportHistory} onToggle={onToggle} onAvailability={onAvailability} onQuantity={onQuantity} onRemove={onRemove} onClear={onClear} onMap={onMap}/>:<div className="order-tab-content"><AssignedOrdersView items={panels.items} history={panels.history} onExportHistory={onExportHistory}/></div>}</div>;
}
