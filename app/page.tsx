...
function DailyReportHistoryView({reports:sourceReports,month,onEdit:_onEdit}:{reports:DailyReport[];month:string;onEdit:(report:DailyReport)=>void}) {
  const [localReports,setLocalReports]=useState(sourceReports),[deletedIds,setDeletedIds]=useState<Set<string>>(new Set()),[deletingId,setDeletingId]=useState(""),[editingId,setEditingId]=useState(""),[editingValues,setEditingValues]=useState<Record<string,string>>({}),[savingId,setSavingId]=useState("");
  useEffect(()=>{setLocalReports(sourceReports);},[sourceReports]);
  const reports=localReports.filter((report)=>!deletedIds.has(report.id)),canDelete=true;
  const startEdit=(report:DailyReport)=>{setEditingId(report.id);setEditingValues(Object.fromEntries(Object.entries(report).map(([key,value])=>[key,String(value??"")])));};
  const cancelEdit=()=>{if(!savingId){setEditingId("");setEditingValues({});}};
  const saveEdit=async(report:DailyReport)=>{if(savingId)return;setSavingId(report.id);try{const response=await fetch("/api/daily-reports",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({report:{...editingValues,id:report.id}})}),result=await readApiJson<{error?:string;report?:DailyReport}>(response);if(!response.ok||!result.report)throw new Error(result.error||"Không thể lưu báo cáo ngày");setLocalReports((current)=>current.map((item)=>item.id===report.id?result.report!:item));setEditingId("");setEditingValues({});}catch(cause){window.alert(cause instanceof Error?cause.message:"Không thể lưu báo cáo ngày");}finally{setSavingId("");}};
  ...
  return <section className="panel daily-report-list">
    <div className="panel-title"><div><h2>Lịch sử nhập báo cáo tháng {month}</h2><span>Chọn Sửa để chỉnh trực tiếp trên dòng; chỉ Manager/Admin có thể xóa.</span></div><b>{reports.length} báo cáo</b></div>
    <div className="table-wrap"><table className="compact-table report-table daily-report-history-table">
      ...
      <tbody>{reports.map((report,index)=>{const isEditing=editingId===report.id;return <tr key={report.id} onClick={()=>{if(!isEditing)startEdit(report);}}>
        <td><div className="report-history-actions">{isEditing?<><button type="button" disabled={savingId===report.id} onClick={(event)=>{event.stopPropagation();void saveEdit(report);}}>{savingId===report.id?"Đang lưu…":"Lưu"}</button><button type="button" disabled={savingId===report.id} onClick={(event)=>{event.stopPropagation();cancelEdit();}}>Hủy</button></>:<button type="button" onClick={(event)=>{event.stopPropagation();startEdit(report);}}>Sửa</button>}{canDelete&&!isEditing&&<button type="button" className="danger-text" disabled={deletingId===report.id} onClick={(event)=>{event.stopPropagation();void onDelete(report);}}>{deletingId===report.id?"Đang xóa…":"Xóa"}</button>}</div></td>
        <td>{index+1}</td>{dailyReportFields.map(([key])=><td key={String(key)}>{isEditing?<input className="history-inline-input" type={key==="date"?"date":"text"} value={editingValues[String(key)]||""} onChange={(event)=>setEditingValues((current)=>({...current,[String(key)]:event.target.value}))} onClick={(event)=>event.stopPropagation()}/>:displayValue(report,key)}</td>)}
...
function DailyReportFieldSections({form,onChange,carrierUsers}:{form:Record<string,string>;onChange:(key:string,value:string)=>void;carrierUsers:UserRole[]}) {
  const compactKeys=new Set(["employeeName","orderType","date","phone","customerName","customerStatus","vatExport","invoiceNumber","invoiceValue","paymentMethod","cdoNumber","codNumber","carrier","returnStatus","remainingInvoiceValue","memberCard","customerGroup","email","taxId"]);
  const renderField=([key,label,isLinked]:typeof dailyReportFields[number])=>{
    ...
    return <label key={String(key)} className={`report-field-${String(key)} ${compact?"compact-report-field ":""}${isLinked?"customer-linked-field ":""}${wide?"wide":""}`}><span>{label}{isLinked&&<i>●</i>}</span>{input}</label>;
  };
...
function DailyReportView({reports,customers,users,month,onMonth,onSave,onImport,onExport,onExportCustomers,canManage,busy,purchaseHistory,purchaseMonth,onPurchaseMonth,onPurchaseImport,onPurchaseExport,onPurchaseDelete,purchaseBusy}:{reports:DailyReport[];customers:CustomerMaster[];users:UserRole[];month:string;onMonth:(value:string)=>void;onSave:(report:Record<string,unknown>)=>Promise<boolean>;onImport:()=>void;onExport:(month:string)=>void;onExportCustomers:()=>void;canManage:boolean;busy:boolean;purchaseHistory:PurchaseHistorySummary;purchaseMonth:string;onPurchaseMonth:(value:string)=>void;onPurchaseImport:()=>void;onPurchaseExport:(month:string)=>void;onPurchaseDelete:(month:string)=>void;purchaseBusy:boolean}) {
