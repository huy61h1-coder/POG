...
export default function Home() {
  ...
  return (
    <main className="ops-shell">
      ...
      {settingsOpen&&<SettingsModal actor={data.actor} users={data.users} theme={theme} appLogo={data.appBrand?.logo||"/aeon-logo.svg"} logoSizeDesktop={desktopLogoSize} logoSizeMobile={mobileLogoSize} onLogo={(logo)=>mutate("updateAppBrand",{logo})} onLogoSize={(device,size)=>mutate("updateAppBrand",{[device==="desktop"?"logoSizeDesktop":"logoSizeMobile"]:size})} onTheme={setTheme} onCreate={(account)=>mutate("createAccount",{account})} onUpdate={(account)=>mutate("updateAccount",{account})} onPassword={(currentPassword,newPassword)=>mutate("changeOwnPassword",{currentPassword,newPassword})} onLogout={logout} onClose={()=>setSettingsOpen(false)}/>}
...
function SettingsModal({actor,users,theme,appLogo,logoSizeDesktop,logoSizeMobile,onLogo,onLogoSize,onTheme,onCreate,onUpdate,onPassword,onLogout,onClose}:{actor:Actor;users:UserRole[];theme:string;appLogo:string;logoSizeDesktop:number;logoSizeMobile:number;onLogo:(logo:string)=>Promise<boolean>;onLogoSize:(device:"desktop"|"mobile",size:number)=>Promise<boolean>;onTheme:(v:string)=>void;onCreate:(account:{name:string;username:string;password:string;role:Role})=>Promise<boolean>;onUpdate:(account:{userId:string;name?:string;role?:Role;active?:boolean;password?:string})=>Promise<boolean>;onPassword:(currentPassword:string,newPassword:string)=>Promise<boolean>;onLogout:()=>Promise<void>;onClose:()=>void}) {
  ...
  const setLogoSize=(device:"desktop"|"mobile",next:number)=>{const min=device==="desktop"?120:72,max=device==="desktop"?320:220;void onLogoSize(device,Math.max(min,Math.min(max,Math.round(next/10)*10)));};
  ...
  return <div className="modal-backdrop"><section className="settings-modal account-settings"><div className="modal-head"><div><p>TÀI KHOẢN</p><h2>Tài khoản & phân quyền</h2></div><button onClick={onClose}>×</button></div>{actor.role==="ADMIN"&&<><h3>Logo ứng dụng</h3><div className="app-logo-setting"><div className="app-logo-previews"><div className="app-logo-preview"><span>Desktop</span><img style={{width:Math.min(320,logoSizeDesktop)+"px"}} src={appLogo} alt="Logo Desktop"/></div><div className="app-logo-preview"><span>Mobile</span><img style={{width:Math.min(220,logoSizeMobile)+"px"}} src={appLogo} alt="Logo Mobile"/></div></div><label className="ghost">Chọn logo<input type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" onChange={(event)=>updateLogo(event.target.files?.[0])}/></label><button className="ghost" onClick={()=>void onLogo("/aeon-logo.svg")}>Khôi phục AEON</button><div className="logo-size-control"><span>Desktop</span><div className="logo-size-stepper"><button type="button" aria-label="Giảm kích thước logo Desktop" disabled={logoSizeDesktop<=120} onClick={()=>setLogoSize("desktop",logoSizeDesktop-10)}>−</button><b>{logoSizeDesktop}px</b><button type="button" aria-label="Tăng kích thước logo Desktop" disabled={logoSizeDesktop>=320} onClick={()=>setLogoSize("desktop",logoSizeDesktop+10)}>+</button></div><input type="range" min="120" max="320" step="10" value={logoSizeDesktop} aria-label="Kích thước logo Desktop" onChange={(event)=>setLogoSize("desktop",Number(event.target.value))}/></div><div className="logo-size-control"><span>Mobile</span><div className="logo-size-stepper"><button type="button" aria-label="Giảm kích thước logo Mobile" disabled={logoSizeMobile<=72} onClick={()=>setLogoSize("mobile",logoSizeMobile-10)}>−</button><b>{logoSizeMobile}px</b><button type="button" aria-label="Tăng kích thước logo Mobile" disabled={logoSizeMobile>=220} onClick={()=>setLogoSize("mobile",logoSizeMobile+10)}>+</button></div><input type="range" min="72" max="220" step="10" value={logoSizeMobile} aria-label="Kích thước logo Mobile" onChange={(event)=>setLogoSize("mobile",Number(event.target.value))}/></div><small>Logo và kích thước từng thiết bị được lưu chung trên hệ thống, áp dụng cho mọi tài khoản.</small></div></>}<h3>Giao diện</h3><div className="theme-row">{themes.map(([color,label])=><button key={color} className={theme===color?"active":""} data-color={color} onClick={()=>onTheme(color)}><i/>{label}</button>)}</div>
