  ...
  return (
    <main className="ops-shell">
      ...
      <header className="ops-topbar">
        <button className="ops-brand" aria-label="AEON Fulfillment SmartOps" onClick={()=>{setTab("DASHBOARD");setProductPage(1);}}><img src={data.appBrand?.logo||"/aeon-logo.svg"} alt="AEON Fulfillment SmartOps"/></button>
      ...
      {settingsOpen&&<SettingsModal actor={data.actor} users={data.users} theme={theme} appLogo={data.appBrand?.logo||"/aeon-logo.svg"} onLogo={(logo)=>mutate("updateAppBrand",{logo})} onTheme={setTheme} onCreate={(account)=>mutate("createAccount",{account})} onUpdate={(account)=>mutate("updateAccount",{account})} onPassword={(currentPassword,newPassword)=>mutate("changeOwnPassword",{currentPassword,newPassword})} onLogout={logout} onClose={()=>setSettingsOpen(false)}/>} 
...
function SettingsModal({actor,users,theme,appLogo,onLogo,onTheme,onCreate,onUpdate,onPassword,onLogout,onClose}:{actor:Actor;users:UserRole[];theme:string;appLogo:string;onLogo:(logo:string)=>Promise<boolean>;onTheme:(v:string)=>void;onCreate:(account:{name:string;username:string;password:string;role:Role})=>Promise<boolean>;onUpdate:(account:{userId:string;name?:string;role?:Role;active?:boolean;password?:string})=>Promise<boolean>;onPassword:(currentPassword:string,newPassword:string)=>Promise<boolean>;onLogout:()=>Promise<void>;onClose:()=>void}) {
