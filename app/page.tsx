...
export default function Home() {
  ...
  const reanalyzeAllPogs = async () => { const records=(data?.pogFiles||[]).filter((file)=>file.mimeType==="application/pdf");if(!records.length){setToast("Chưa có PDF POG để áp dụng.");return;}setBusy(true);let completed=0;try{for(const record of records){const files=await loadPogSourceFiles(record),analysis=await analyzePogFiles(files);if(!analysis)continue;await savePogAnalysis(files[0],record.line,record.side,analysis,"reanalyze");completed++;}await loadData(true);setToast(`Đã áp dụng cơ chế ghép POG cho ${completed}/${records.length} Line/mặt có PDF.`);}catch(cause){setToast(cause instanceof Error?cause.message:"Không thể ghép lại toàn bộ POG");}finally{setBusy(false);} };
    ...
    <main className="ops-shell">
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {tab==="DASHBOARD"&&<Dashboard products={data.alertProducts.map(withPogLocation)} totalProducts={data.productTotal} logs={data.logs} totals={data.productStats} onGo={(next)=>{setTab(next);setProductPage(1);}}/>}
          {tab==="MAP"&&
            <MapView pogFiles={data.pogFiles} lineConfigs={data.lineConfigs||[]} query={query} canManage={canManage(data.actor.role)} pogPdfCount={data.pogFiles.filter((file)=>file.mimeType==="application/pdf").length} onReanalyzeAll={()=>void reanalyzeAllPogs()} onOpen={(line,side)=>setPogModal({line,side})} onEdit={(lineConfig)=>setLineModal(lineConfig)}/>
          }
...
function MapView({pogFiles,lineConfigs,query,canManage,pogPdfCount,onReanalyzeAll,onOpen,onEdit}:{pogFiles:PogFile[];lineConfigs:LineConfig[];query:string;canManage:boolean;pogPdfCount:number;onReanalyzeAll:()=>void;onOpen:(line:string,side:"A"|"B")=>void;onEdit:(lineConfig:LineConfig)=>void}) {
  ...
  return <div><PageHead eyebrow="BẢN ĐỒ CỬA HÀNG" title="Sơ đồ POG" subtitle="Chọn dãy và mặt kệ để xem vị trí sản phẩm chi tiết." actions={canManage&&pogPdfCount>0?<button className="ghost" onClick={onReanalyzeAll}>↻ Áp dụng ghép POG cho tất cả ({pogPdfCount})</button>:undefined}/>
...
function AssignPickModal({product,users,customerNames,onClose,onAssign}:{product:Product;users:UserRole[];customerNames:string[];onClose:()=>void;onAssign:(assignment:{assigneeId:string;quantity:number;customerName:string;note:string})=>Promise<void>}) {
...
function AssignedOrdersView({items}:{items:AssignedPickItem[]}) {
  ...
  return <section className="assigned-orders"><div className="panel-title"><h2>ĐƠN ĐÃ GIAO THEO KHÁCH HÀNG</h2><span>{grouped.size} khách · {items.length} mặt hàng</span></div>{[...grouped.entries()].map(([key,orders])=>{const totalQuantity=orders.reduce((sum,item)=>sum+item.quantity,0);const pickedQuantity=orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0);return <article key={key}><header><b>Khách: {orders[0].customerName||"Chưa đặt tên"}</b><span>{totalQuantity} mặt hàng · {pickedQuantity}/{totalQuantity} đã lấy</span></header>{orders.map((item)=><div key={item.pickId}><em className={item.picked?"done":""}>{item.picked?"✓":"•"}</em><p><b>{item.name}</b><small>Nhân viên: {item.assigneeName||"—"} · SL {item.quantity} · SKU {item.sku}{item.note?" · Note: "+item.note:""}</small></p><strong>{item.shelfLine?`POG ${item.shelfLine}${item.shelfSide} · ${item.shelfPosition}`:"Chưa gán POG"}</strong></div>)}</article>})}{!items.length&&<div className="empty"><b>Chưa có đơn nào được giao.</b></div>}</section>;
}
...
function SettingsModal({actor,users,theme,appLogo,logoSizeDesktop,logoSizeMobile,onSaveAppearance,onCreate,onUpdate,onPassword,onLogout,onClose}:{actor:Actor;users:UserRole[];theme:string;appLogo:string;logoSizeDesktop:number;logoSizeMobile:number;onSaveAppearance:(changes:{logo:string;logoSizeDesktop:number;logoSizeMobile:number;theme:string})=>Promise<boolean>;onCreate:(account:{name:string;username:string;password:string;role:Role})=>Promise<boolean>;onUpdate:(account:{userId:string;name?:string;role?:Role;active?:boolean;password?:string})=>Promise<boolean>;onPassword:(currentPassword:string,newPassword:string)=>Promise<boolean>;onLogout:()=>Promise<void>;onClose:()=>void}) {
  ...
  const [draftLogo,setDraftLogo]=useState(appLogo),[draftLogoDesktop,setDraftLogoDesktop]=useState(logoSizeDesktop),[draftLogoMobile,setDraftLogoMobile]=useState(logoSizeMobile),[draftTheme,setDraftTheme]=useState(theme),[savingAppearance,setSavingAppearance]=useState(false);
  ...
  const appearanceDirty=draftLogo!==appLogo||draftLogoDesktop!==logoSizeDesktop||draftLogoMobile!==logoSizeMobile||draftTheme!==theme;
  const saveAppearance=async()=>{if(!appearanceDirty||savingAppearance)return;setSavingAppearance(true);try{if(await onSaveAppearance({logo:draftLogo,logoSizeDesktop:draftLogoDesktop,logoSizeMobile:draftLogoMobile,theme:draftTheme}))setSavingAppearance(false);}finally{setSavingAppearance(false);}};
  return <div className="modal-backdrop"><section className="settings-modal account-settings"><div className="modal-head"><div><p>TÀI KHOẢN</p><h2>Tài khoản & phân quyền</h2></div><button onClick={onClose}>×</button></div>{actor.role==="ADMIN"&&<><h3>Logo ứng dụng</h3><div className="app-logo-setting"><div className="app-logo-previews"><div className="app-logo-preview"><span>Desktop</span><img style={{width:Math.min(320,draftLogoDesktop)+"px"}} src={draftLogo} alt="Logo Desktop"/></div><div className="app-logo-preview"><span>Mobile</span><img style={{width:Math.min(220,draftLogoMobile)+"px"}} src={draftLogo} alt="Logo Mobile"/></div></div><label className="ghost">Chọn logo<input type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" onChange={(event)=>updateLogo(event.target.files?.[0])}/></label><button className="ghost" onClick={()=>setDraftLogo("/aeon-logo.svg")}>Khôi phục AEON</button><div className="logo-size-control"><span>Desktop</span><div className="logo-size-stepper"><button type="button" aria-label="Giảm kích thước logo Desktop" disabled={draftLogoDesktop<=50} onClick={()=>setLogoSize("desktop",draftLogoDesktop-10)}>−</button><b>{draftLogoDesktop}px</b><button type="button" aria-label="Tăng kích thước logo Desktop" disabled={draftLogoDesktop>=320} onClick={()=>setLogoSize("desktop",draftLogoDesktop+10)}>+</button></div><input type="range" min="50" max="320" step="10" value={draftLogoDesktop} aria-label="Kích thước logo Desktop" onChange={(event)=>setLogoSize("desktop",Number(event.target.value))}/></div><div className="logo-size-control"><span>Mobile</span><div className="logo-size-stepper"><button type="button" aria-label="Giảm kích thước logo Mobile" disabled={draftLogoMobile<=72} onClick={()=>setLogoSize("mobile",draftLogoMobile-10)}>−</button><b>{draftLogoMobile}px</b><button type="button" aria-label="Tăng kích thước logo Mobile" disabled={draftLogoMobile>=220} onClick={()=>setLogoSize("mobile",draftLogoMobile+10)}>+</button></div><input type="range" min="72" max="220" step="10" value={draftLogoMobile} aria-label="Kích thước logo Mobile" onChange={(event)=>setLogoSize("mobile",Number(event.target.value))}/></div><small>Logo và kích thước từng thiết bị chỉ được lưu khi bấm “Lưu thay đổi”.</small></div></>}<h3>Giao diện</h3><div className="theme-row">{themes.map(([color,label])=><button key={color} className={draftTheme===color?"active":""} data-color={color} onClick={()=>setDraftTheme(color)}><i/>{label}</button>)}</div>
    ...
    <div className="modal-actions"><button className="ghost danger-text" onClick={()=>void onLogout()}>Đăng xuất</button><span className="settings-save-note">{appearanceDirty?"Có thay đổi chưa lưu":"Đã lưu"}</span><button className="ghost" onClick={onClose}>Đóng</button><button className="primary" disabled={!appearanceDirty||savingAppearance} onClick={()=>void saveAppearance()}>{savingAppearance?"Đang lưu…":"Lưu thay đổi"}</button></div></section></div>;
}
...
function PogModal({modal,setModal,products,total,file,search,setSearch,canUpload,uploadRef,onUpload,onAppend,onReanalyze,onPageChange,onPick,onClose}:{modal:{line:string;side:"A"|"B";selectedId?:string};setModal:(v:{line:string;side:"A"|"B";selectedId?:string})=>void;products:Product[];total:number;file?:PogFile;search:string;setSearch:(v:string)=>void;canUpload:boolean;uploadRef:React.RefObject<HTMLInputElement|null>;onUpload:(f?:File)=>void;onAppend:(f?:File)=>void;onReanalyze:()=>void;onPageChange:(page:number)=>void;onPick:(p:Product)=>void;onClose:()=>void}) {
  ...
  return <div className="modal-backdrop pog-backdrop"><section className="pog-modal"><div className="pog-head"><div><p>SƠ ĐỒ KỆ CHI TIẾT</p><h2>Line {modal.line} · {aisleNames[modal.line]||"Khu vực"}</h2></div><div className="side-switch"><button className={modal.side==="A"?"active":""} onClick={()=>switchSide("A")}>Mặt A</button><button className={modal.side==="B"?"active":""} onClick={()=>switchSide("B")}>Mặt B</button></div>{file?.mimeType==="application/pdf"&&!file.shelfImage&&<label className="pdf-page-picker">Trang sơ đồ<select value={pdfPage} onChange={(e)=>{const page=Number(e.target.value);setPdfPage(page);onPageChange(page)}}>{Array.from({length:12},(_,index)=><option key={index+1} value={index+1}>Trang {index+1}{index<2?" · thường dùng":""}</option>)}</select></label>}{file?.shelfImage&&<span className="pog-linked-count">Đã ghép {fileCount} file · {file.positions?.length||0} vị trí</span>}{canUpload&&file?.mimeType==="application/pdf"&&<button className="upload-pog reanalyze-pog" title="Tạo lại ảnh ghép từ tất cả PDF đang lưu" onClick={onReanalyze}>↻ Ghép lại sát viền</button>}{canUpload&&file&&<button className="upload-pog add-pog" title="Giữ POG hiện có và ghép thêm một PDF" onClick={()=>appendRef.current?.click()}>＋ Thêm file POG</button>}{canUpload&&<button className="upload-pog" title="Thay toàn bộ POG bằng PDF hoặc ảnh mới" onClick={()=>uploadRef.current?.click()}>↑ {file?"Thay POG":"Tải POG PDF/ảnh"}</button>}<button className="close-pog" onClick={onClose}>×</button></div>
    <div className={`pog-body pog-canvas-layout${searchExpanded?" pog-search-expanded":""}`}>
