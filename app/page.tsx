...
type Product = { id:string; sku:string; name:string; division:string; divisionName:string; department:string; departmentName:string; supplierBarcode:string; barcode:string; line:string; lineName:string; side:"A"|"B"; bay:number; price:number; stock:number; sales?:number; stockKnown?:boolean; loss:number; expDate:string; updatedAt?:number };
type PickItem = Product & { pickId:string; quantity:number; picked:boolean|number; customerName:string; note:string; assignedBy:string };
    ...
    <main className="ops-shell">
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {tab==="ORDER"&&<OrderView items={data.picking} assignedItems={data.assignedPicking} role={data.actor.role} onToggle={(p)=>void mutate("togglePick",{pickId:p.pickId})} onQuantity={(p,quantity)=>void mutate("updatePickQuantity",{pickId:p.pickId,quantity})} onRemove={(p)=>void mutate("removePick",{pickId:p.pickId})} onClear={()=>void mutate("clearPick")} onMap={openProductOnMap}/>}
...
function OrderView({items,assignedItems,role,onToggle,onQuantity,onRemove,onClear,onMap}:{items:PickItem[];assignedItems:AssignedPickItem[];role:Role;onToggle:(p:PickItem)=>void;onQuantity:(p:PickItem,quantity:number)=>void;onRemove:(p:PickItem)=>void;onClear:()=>void;onMap:(p:PickItem)=>void}) {
  ...
  return <div><PageHead eyebrow="PICKING LIST" title="Đơn đang soạn" subtitle={pickedUnits+"/"+totalUnits+" sản phẩm đã lấy · sắp theo lộ trình Line"} actions={items.length?<button className="primary" onClick={finish}>{next?"Kết thúc sớm":"Hoàn tất đơn"}</button>:undefined}/>{next&&<section className="next-pick"><div><small>ĐIỂM LẤY TIẾP THEO</small><b>Line {next.line}{next.side} · Kệ {next.bay}</b><span>{next.name} · SL {next.quantity}</span></div><button onClick={()=>onMap(next)}>Mở vị trí →</button></section>}<div className="order-progress-large"><i><span style={{width:percent+"%"}}/></i><b>{percent}%</b></div>
    <div className="order-list">{sorted.map((p)=><article key={p.pickId} className={p.picked?"picked":""}><button className="pick-check" aria-label={p.picked?"Đánh dấu chưa lấy":"Đánh dấu đã lấy"} onClick={()=>onToggle(p)}>{p.picked?"✓":""}</button><div><small>SKU {p.sku}{p.customerName?" · Khách: "+p.customerName:""}</small><b>{p.name}</b><span>Line {p.line}{p.side} · Kệ {p.bay}{p.note?" · "+p.note:""}</span></div><div className="pick-quantity"><button disabled={p.quantity<=1} onClick={()=>onQuantity(p,p.quantity-1)}>−</button><b>{p.quantity}</b><button onClick={()=>onQuantity(p,p.quantity+1)}>+</button></div><StockBadge stock={p.stock}/><button onClick={()=>onMap(p)}>Vị trí</button><button className="danger-text" onClick={()=>onRemove(p)}>Bỏ</button></article>)}{!items.length&&<div className="empty big"><b>Đơn soạn đang trống</b><span>Chờ đơn được giao từ Check Stock hoặc thêm sản phẩm từ tìm kiếm.</span></div>}</div>{canManage(role)&&<AssignedOrdersView items={assignedItems}/>}</div>;
}
function AssignedOrdersView({items}:{items:AssignedPickItem[]}) {
  const grouped=new Map<string,AssignedPickItem[]>();for(const item of items){const key=item.assigneeId;grouped.set(key,[...(grouped.get(key)||[]),item]);}
  return <section className="assigned-orders"><div className="panel-title"><h2>ĐƠN ĐÃ GIAO THEO NHÂN VIÊN</h2><span>{items.length} dòng đơn</span></div>{[...grouped.entries()].map(([,orders])=><article key={orders[0].assigneeId}><header><b>{orders[0].assigneeName}</b><span>{orders.filter((item)=>item.picked).length}/{orders.length} đã lấy</span></header>{orders.map((item)=><div key={item.pickId}><em className={item.picked?"done":""}>{item.picked?"✓":"•"}</em><p><b>{item.name}</b><small>Khách: {item.customerName||"—"} · SL {item.quantity} · SKU {item.sku}{item.note?" · Note: "+item.note:""}</small></p><strong>Line {item.line}{item.side}</strong></div>)}</article>)}{!items.length&&<div className="empty"><b>Chưa có đơn nào được giao.</b></div>}</section>;
}
