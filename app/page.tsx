...
function OrderView({items,assignedItems,history,onExportHistory,onToggle,onAvailability,onQuantity,onRemove,onClear,onMap}:{items:PickItem[];assignedItems:AssignedPickItem[];history:OrderHistoryItem[];onExportHistory:(month:string)=>void;onToggle:(p:PickItem)=>void;onAvailability:(p:PickItem)=>void;onQuantity:(p:PickItem,quantity:number)=>void;onRemove:(p:PickItem)=>void;onClear:()=>void;onMap:(p:PickItem)=>void}) {
  const [tab,setTab]=useState<"unpicked"|"working"|"completed"|"delivered">("working");
  const orderKey=(item:PickItem)=>{const phone=(item.customerPhone||"").replace(/\D/g,""),name=normalize((item.customerName||"").trim())||"chua dat ten khach";return `${orderDateKey(item.orderDate||item.createdAt)}|${phone?`phone:${phone}`:`name:${name}`}`;};
  const groupedOrders=new Map<string,AssignedPickItem[]>();
  for(const item of assignedItems){const key=orderKey(item);groupedOrders.set(key,[...(groupedOrders.get(key)||[]),item]);}
  const orderGroups=[...groupedOrders.values()],flatten=(groups:AssignedPickItem[][])=>groups.flat();
  const unpickedGroups=orderGroups.filter((group)=>group.every((item)=>!item.picked)),workingGroups=orderGroups.filter((group)=>group.some((item)=>item.picked)&&group.some((item)=>!item.picked)),completedGroups=orderGroups.filter((group)=>group.every((item)=>Boolean(item.picked)));
  const unpickedItems=flatten(unpickedGroups),workingItems=flatten(workingGroups),completedItems=flatten(completedGroups),deliveredOrderCount=new Set(history.map(orderKey)).size;
  const panels=tab==="delivered"?{items:[],history}:{items:tab==="unpicked"?unpickedItems:tab==="completed"?completedItems:workingItems,history:[]};
  const panelTitle=tab==="unpicked"?"ĐƠN CHƯA SOẠN":"ĐƠN ĐÃ SOẠN XONG";
  return <div className={`order-tab-shell order-tab-${tab}`}><nav className="order-tabs" aria-label="Trạng thái đơn hàng"><button type="button" className={tab==="unpicked"?"active":""} onClick={()=>setTab("unpicked")} aria-pressed={tab==="unpicked"}><span>Chưa soạn</span><b>{unpickedGroups.length}</b></button><button type="button" className={tab==="working"?"active":""} onClick={()=>setTab("working")} aria-pressed={tab==="working"}><span>Đang soạn</span><b>{workingGroups.length}</b></button><button type="button" className={tab==="completed"?"active":""} onClick={()=>setTab("completed")} aria-pressed={tab==="completed"}><span>Đã soạn xong</span><b>{completedGroups.length}</b></button><button type="button" className={tab==="delivered"?"active":""} onClick={()=>setTab("delivered")} aria-pressed={tab==="delivered"}><span>Đã giao</span><b>{deliveredOrderCount}</b></button></nav>{tab==="working"?<OrderViewLegacy items={items.filter((item)=>workingGroups.some((group)=>orderKey(group[0])===orderKey(item)))} assignedItems={assignedItems} history={history} onExportHistory={onExportHistory} onToggle={onToggle} onAvailability={onAvailability} onQuantity={onQuantity} onRemove={onRemove} onClear={onClear} onMap={onMap}/>:<div className="order-tab-content"><AssignedOrdersView items={panels.items} history={panels.history} assignedTitle={panelTitle} historyTitle="ĐƠN ĐÃ GIAO" onExportHistory={onExportHistory}/></div>}</div>;
}
function AssignedOrdersView({items,history,assignedTitle="ĐƠN ĐÃ GIAO THEO KHÁCH HÀNG",historyTitle="LỊCH SỬ ĐƠN HÀNG",onExportHistory}:{items:AssignedPickItem[];history:OrderHistoryItem[];assignedTitle?:string;historyTitle?:string;onExportHistory:(month:string)=>void}) {
  const [historyDate,setHistoryDate]=useState(""),[historyCustomer,setHistoryCustomer]=useState(""),[exportMonth,setExportMonth]=useState(defaultOrderMonth),[selectedGroup,setSelectedGroup]=useState<{key:string;orders:PickItem[];historyMode:boolean}|null>(null);
  const groupItems=<T extends PickItem>(rows:T[])=>{const grouped=new Map<string,T[]>();for(const item of rows){const name=(item.customerName||"Chưa đặt tên khách").trim().replace(/\s+/g," "),phone=(item.customerPhone||"").replace(/\D/g,""),date=orderDateKey(item.orderDate||item.createdAt),groupKey=phone?`phone:${phone}`:`name:${normalize(name)||"chua dat ten khach"}`,key=[date,groupKey].join("|");grouped.set(key,[...(grouped.get(key)||[]),{...item,customerName:name}]);}return grouped;};
  ...
  const renderGroup=(key:string,orders:PickItem[],historyMode=false)=>{const totalQuantity=orders.reduce((sum,item)=>sum+item.quantity,0),pickedQuantity=orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0),first=orders[0],date=orderDateKey(first.orderDate||((first as OrderHistoryItem).completedAt)),groupId=(historyMode?"history|":"assigned|")+key;return <article key={groupId} className="customer-order-group"><button type="button" className="customer-order-header" onClick={()=>setSelectedGroup({key,orders,historyMode})} aria-label={`Xem chi tiết đơn của ${first.customerName||"khách hàng"}`}><span><b>Khách: {first.customerName||"Chưa đặt tên"}</b>{first.customerPhone&&<small>☎ {first.customerPhone}</small>}<small>{orderDateLabel(date)}</small></span><strong>{totalQuantity} mặt hàng · {historyMode?"Đã hoàn tất":pickedQuantity+"/"+totalQuantity+" đã lấy"}<i>›</i></strong></button></article>;};
  const selectedFirst=selectedGroup?.orders[0],selectedDate=selectedFirst?orderDateKey(selectedFirst.orderDate||((selectedFirst as OrderHistoryItem).completedAt)):"";
  return <><section className="assigned-orders"><div className="panel-title"><h2>{assignedTitle}</h2><span>{assignedGroups.size} khách/ngày · {items.length} mặt hàng</span></div>{[...assignedGroups.entries()].map(([key,orders])=>renderGroup(key,orders))}{!items.length&&<div className="empty"><b>Chưa có đơn nào được giao.</b><span>Các sản phẩm cùng số điện thoại sẽ được gom chung một đơn trong cùng ngày.</span></div>}</section><section className="order-history-panel"><div className="panel-title"><h2>{historyTitle}</h2><span>{historyGroups.size} khách/ngày · {history.length} mặt hàng</span></div><div className="order-history-toolbar"><label>Tìm khách hàng<input type="search" value={historyCustomer} onChange={(event)=>setHistoryCustomer(event.target.value)} placeholder="Tên hoặc số điện thoại"/></label><label>Chọn ngày xem lại<input type="date" value={historyDate} onChange={(event)=>setHistoryDate(event.target.value)}/></label><label>Tháng lưu Excel<input type="month" value={exportMonth} onChange={(event)=>setExportMonth(event.target.value)}/></label><button type="button" className="primary" disabled={!exportMonth} onClick={()=>onExportHistory(exportMonth)}>↓ Xuất Excel</button><button type="button" className="ghost" disabled={!historyDate&&!historyCustomer} onClick={()=>{setHistoryDate("");setHistoryCustomer("")}}>Tất cả</button></div>{historyGroups.size?[...historyGroups.entries()].map(([key,orders])=>renderGroup(key,orders,true)):<div className="empty"><b>{historyDate||historyCustomer?"Không có đơn phù hợp":"Chưa có lịch sử đơn hàng"}</b><span>Đơn cùng số điện thoại được lưu chung theo từng ngày.</span></div>}</section>{selectedGroup&&selectedFirst&&<div className="modal-backdrop order-detail-backdrop"><section className="order-detail-modal" role="dialog" aria-modal="true" aria-label="Chi tiết đơn hàng"><div className="modal-head"><div><p>{selectedGroup.historyMode?"LỊCH SỬ ĐƠN HÀNG":"ĐƠN ĐANG SOẠN"}</p><h2>{selectedFirst.customerName||"Chưa đặt tên khách"}</h2></div><button type="button" onClick={()=>setSelectedGroup(null)} aria-label="Đóng">×</button></div><div className="order-detail-meta"><span><small>Số điện thoại</small><b>{selectedFirst.customerPhone||"—"}</b></span><span><small>Ngày đơn</small><b>{orderDateLabel(selectedDate)}</b></span><span><small>Tổng mặt hàng</small><b>{selectedGroup.orders.reduce((sum,item)=>sum+item.quantity,0)}</b></span><span><small>Trạng thái</small><b>{selectedGroup.historyMode?"Đã hoàn tất":`${selectedGroup.orders.filter((item)=>item.picked).reduce((sum,item)=>sum+item.quantity,0)}/${selectedGroup.orders.reduce((sum,item)=>sum+item.quantity,0)} đã lấy`}</b></span></div><div className="order-detail-items"><div className="order-detail-items-head"><b>Danh sách hàng trong đơn</b><span>{selectedGroup.orders.length} dòng sản phẩm</span></div>{selectedGroup.orders.map((item,index)=><article key={item.pickId}><em>{index+1}</em><div><b>{item.name}</b><small>SKU {item.sku} · SL {item.quantity}{item.barcode||item.supplierBarcode?` · Barcode ${item.barcode||item.supplierBarcode}`:""}</small><small>{selectedGroup.historyMode?`Hoàn tất bởi: ${(item as OrderHistoryItem).completedBy||"—"}`:`Nhân viên: ${(item as AssignedPickItem).assigneeName||"—"}`}{item.note?` · ${item.note}`:""}</small></div><strong>{item.shelfLine?`POG ${item.shelfLine}${item.shelfSide} · ${item.shelfPosition}`:"Chưa gán POG"}</strong></article>)}</div><div className="modal-actions"><button type="button" className="primary" onClick={()=>setSelectedGroup(null)}>Đóng chi tiết</button></div></section></div>}</>;
}
