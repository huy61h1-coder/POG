...
function ProductsView({products,total,role,importResult,importJob,onAdd,onEdit,onDelete,onMap,onPick,onExport,onImport}:{products:Product[];total:number;role:Role;importResult:MasterImportResult|null;importJob:MasterImportJob|null;onAdd:()=>void;onEdit:(p:Product)=>void;onDelete:(p:Product)=>void;onMap:(p:Product)=>void;onPick:(p:Product)=>void;onExport:()=>void;onImport:()=>void}) {
  ...
  return <div><PageHead eyebrow="MASTER DATA" title="Master Data sản phẩm" subtitle={money.format(total)+" SKU phù hợp"} actions={<><button className="ghost" onClick={onExport}>↓ Xuất Master CSV</button>{canManage(role)&&<button className="ghost" disabled={importing} onClick={onImport}>{importing?"Đang nhập Excel…":"↑ Nhập Excel (.xlsx)"}</button>}{canManage(role)&&<button className="primary" onClick={onAdd}>+ Thêm sản phẩm</button>}</>}/>
    ...
    <div className="table-wrap"><table className="master-table"><thead><tr><th>ẢNH</th><th>SKU</th><th>TÊN SẢN PHẨM</th><th>Sale</th><th>Stock</th><th>GIÁ BÁN RETAIL</th><th>GIÁ KHUYẾN MÃI</th><th>Division</th><th>DIVISION NAME</th><th>Department</th><th>DEPARTMENT NAME</th><th>BARCODE NCC</th><th>BARCODE AEON</th><th>VỊ TRÍ KỆ POG</th><th/></tr></thead><tbody>{products.map((p)=><tr key={p.id}>
      <td data-label="ẢNH"><div className="product-table-image">{p.imageUrl?<img src={p.imageUrl} alt={p.name}/>:<span>Chưa có ảnh</span>}</div></td>
      <td data-label="SKU"><b>{p.sku}</b></td>
      <td data-label="TÊN SẢN PHẨM"><strong>{p.name}</strong></td>
      <td data-label="Sale">{money.format(p.sales??0)}</td>
      <td data-label="Stock"><StockBadge stock={p.stock}/></td>
      <td data-label="GIÁ BÁN RETAIL">{money.format(p.price??0)} đ</td>
      <td data-label="GIÁ KHUYẾN MÃI">{p.promoPrice?money.format(p.promoPrice)+" đ":"—"}</td>
      ...
      <td data-label="DEPARTMENT NAME">{p.departmentName||"—"}</td>
      <td data-label="BARCODE NCC"><b>{p.supplierBarcode||"—"}</b></td>
      <td data-label="BARCODE AEON"><b>{p.barcode||"—"}</b></td>
    ...
    </tr>)}{!products.length&&<tr><td colSpan={15}><div className="empty big"><b>Chưa có sản phẩm phù hợp</b><span>Thử đổi bộ lọc hoặc nhập Master Data từ Excel.</span></div></td></tr>}</tbody></table></div></div>;
}
function StockCheckView({products,total,role,metadata,job,onImport,onExport,onAssign}:{products:Product[];total:number;role:Role;metadata?:StoreData["stockImport"];job:StockImportJob|null;onImport:()=>void;onExport:()=>void;onAssign:(product:Product)=>void}) {
  ...
  return <div><PageHead eyebrow="TỒN KHO TỪ FILE UPLOAD" title="Check Stock" subtitle="Chỉ hiển thị số tồn có trong file Stock đã upload; không lấy từ Master Data." actions={<>{products.length>0&&<button onClick={onExport}>↓ Xuất Stock</button>}{canManage(role)&&<button className="primary" disabled={active} onClick={onImport}>{active?"Đang xử lý…":"↑ Upload Stock .xlsx"}</button>}</>}/>
    <div className="master-help"><b>Định dạng file Stock</b><span>Thứ tự chuẩn: SKU · Tên sản phẩm · Sale · Stock · Giá bán retail · Giá khuyến mãi · Division · Division Name · Department · Department Name · Barcode NCC · Barcode AEON. Hệ thống nhận đúng theo tên cột dù file cũ có thứ tự khác; chỉ SKU, Sale và Stock dùng để cập nhật tồn. Vị trí kệ chỉ lấy từ POG đã liên kết.</span></div>
    ...
    <div className="table-wrap"><table className="master-table"><thead><tr><th>SKU</th><th>TÊN SẢN PHẨM</th><th>Sale</th><th>Stock</th><th>GIÁ BÁN RETAIL</th><th>GIÁ KHUYẾN MÃI</th><th>Division</th><th>DIVISION NAME</th><th>Department</th><th>DEPARTMENT NAME</th><th>BARCODE NCC</th><th>BARCODE AEON</th><th>VỊ TRÍ KỆ POG</th>{canManage(role)&&<th/>}</tr></thead><tbody>{products.map((p)=><tr key={p.id}><td><b>{p.sku}</b></td><td><strong>{p.name}</strong></td><td>{money.format(p.sales??0)}</td><td><StockBadge stock={p.stock}/></td><td>{money.format(p.price??0)} đ</td><td>{p.promoPrice?money.format(p.promoPrice)+" đ":"—"}</td><td>{p.division||"—"}</td><td>{p.divisionName||"—"}</td><td>{p.department||"—"}</td><td>{p.departmentName||"—"}</td><td><b>{p.supplierBarcode||"—"}</b></td><td><b>{p.barcode||"—"}</b></td><td>{p.shelfLine?`Line ${p.shelfLine}${p.shelfSide} · Vị trí ${p.shelfPosition}`:"Chưa gán POG"}</td>{canManage(role)&&<td><button className="primary assign-button" disabled={p.stock<=0} onClick={()=>onAssign(p)}>Gán đơn</button></td>}</tr>)}{!products.length&&<tr><td colSpan={canManage(role)?14:13}><div className="empty big"><b>Chưa có file Stock</b><span>Upload file Stock để bắt đầu xem tồn kho tách biệt.</span></div></td></tr>}</tbody></table></div><p className="stock-total">{money.format(total)} SKU có trong file Stock</p></div>;
}
