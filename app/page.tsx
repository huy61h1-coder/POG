...
export default function Home() {
  ...
  const [appLogo,setAppLogo] = useState(()=>typeof window!=="undefined"?window.localStorage.getItem("pog-app-logo")||"/aeon-logo.svg":"/aeon-logo.svg");
  ...
  return (
    <main className="ops-shell">
      ...
      <header className="ops-topbar">
        <button className="ops-brand" aria-label="AEON Fulfillment SmartOps" onClick={()=>{setTab("DASHBOARD");setProductPage(1);}}><img src={appLogo} alt="AEON Fulfillment SmartOps"/></button>
      ...
      {settingsOpen&&<SettingsModal actor={data.actor} users={data.users} theme={theme} appLogo={appLogo} onLogo={(logo)=>{setAppLogo(logo);window.localStorage.setItem("pog-app-logo",logo)}} onTheme={setTheme} onCreate={(account)=>mutate("createAccount",{account})} onUpdate={(account)=>mutate("updateAccount",{account})} onPassword={(currentPassword,newPassword)=>mutate("changeOwnPassword",{currentPassword,newPassword})} onLogout={logout} onClose={()=>setSettingsOpen(false)}/>} 
...
function SettingsModal({actor,users,theme,appLogo,onLogo,onTheme,onCreate,onUpdate,onPassword,onLogout,onClose}:{actor:Actor;users:UserRole[];theme:string;appLogo:string;onLogo:(logo:string)=>void;onTheme:(v:string)=>void;onCreate:(account:{name:string;username:string;password:string;role:Role})=>Promise<boolean>;onUpdate:(account:{userId:string;name?:string;role?:Role;active?:boolean;password?:string})=>Promise<boolean>;onPassword:(currentPassword:string,newPassword:string)=>Promise<boolean>;onLogout:()=>Promise<void>;onClose:()=>void}) {
  ...
  return <div className="modal-backdrop"><section className="settings-modal account-settings"><div className="modal-head"><div><p>TÀI KHOẢN</p><h2>Tài khoản & phân quyền</h2></div><button onClick={onClose}>×</button></div><h3>Logo ứng dụng</h3><div className="app-logo-setting"><img src={appLogo} alt="Logo hiện tại"/><label className="ghost">Chọn logo<input type="file" accept="image/png,image/jpeg,image/svg+xml,image/webp" onChange={(event)=>{const file=event.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>onLogo(String(reader.result));reader.readAsDataURL(file);}}/></label><button className="ghost" onClick={()=>onLogo("/aeon-logo.svg")}>Khôi phục AEON</button></div><h3>Giao diện</h3><div className="theme-row">{themes.map(([color,label])=><button key={color} className={theme===color?"active":""} data-color={color} onClick={()=>onTheme(color)}><i/>{label}</button>)}</div>
