...
function DailyReportFieldSections({form,onChange}:{form:Record<string,string>;onChange:(key:string,value:string)=>void}) {
  ...
  const renderField=([key,label,isLinked]:typeof dailyReportFields[number])=>{
    ...
    return <label key={String(key)} className={(compact?"compact-report-field ":"")+(isLinked?"customer-linked-field ":"")+(wide?"wide":"")}><span>{label}{isLinked&&<i>●</i>}</span><input type={type} value={form[String(key)]||""} onChange={(event)=>onChange(String(key),event.target.value)} placeholder={key==="phone"?"Nhập SĐT để tra cứu khách hàng":label}/></label>;
  };
