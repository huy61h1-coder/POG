...
function PogModal({modal,setModal,products,total,file,search,setSearch,canUpload,uploadRef,onUpload,onAppend,onReanalyze,onPageChange,onPick,onClose}:{modal:{line:string;side:"A"|"B";selectedId?:string};setModal:(v:{line:string;side:"A"|"B";selectedId?:string})=>void;products:Product[];total:number;file?:PogFile;search:string;setSearch:(v:string)=>void;canUpload:boolean;uploadRef:React.RefObject<HTMLInputElement|null>;onUpload:(f?:File)=>void;onAppend:(f?:File)=>void;onReanalyze:()=>void;onPageChange:(page:number)=>void;onPick:(p:Product)=>void;onClose:()=>void}) {
  ...
  return <div className="modal-backdrop pog-backdrop"><section className="pog-modal"><div className="pog-head"><div><p>SƠ ĐỒ KỆ CHI TIẾT</p><h2>Line {modal.line} · {aisleNames[modal.line]||"Khu vực"}</h2></div><div className="side-switch"><button className={modal.side==="A"?"active":""} onClick={()=>switchSide("A")}>Mặt A</button><button className={modal.side==="B"?"active":""} onClick={()=>switchSide("B")}>Mặt B</button></div>{file?.mimeType==="application/pdf"&&!file.shelfImage&&<label className="pdf-page-picker">Trang sơ đồ<select value={pdfPage} onChange={(e)=>{const page=Number(e.target.value);setPdfPage(page);onPageChange(page)}}>{Array.from({length:12},(_,index)=><option key={index+1} value={index+1}>Trang {index+1}{index<2?" · thường dùng":""}</option>)}</select></label>}{file?.shelfImage&&<span className="pog-linked-count">Đã ghép {fileCount} file · {file.positions?.length||0} vị trí</span>}{canUpload&&file?.mimeType==="application/pdf"&&<button className="upload-pog reanalyze-pog" title="Tạo lại ảnh ghép từ tất cả PDF đang lưu" onClick={onReanalyze}>↻ Ghép lại sát viền</button>}{canUpload&&file&&<button className="upload-pog add-pog" title="Giữ POG hiện có và ghép thêm một PDF" onClick={()=>appendRef.current?.click()}>＋ Thêm file POG</button>}{canUpload&&<button className="upload-pog" title="Thay toàn bộ POG bằng PDF hoặc ảnh mới" onClick={()=>uploadRef.current?.click()}>↑ {file?"Thay POG":"Tải POG PDF/ảnh"}</button>}<button className="close-pog" onClick={onClose}>×</button></div>
    <div className="pog-body pog-canvas-layout"><aside className="pog-list"><label>⌕<input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Tìm SKU, barcode, tên…"/><b>{products.length}/{total} SP</b></label>{total>products.length&&<p className="pog-limit-note">Đang hiện 200 kết quả đầu · nhập SKU hoặc tên để tìm chính xác.</p>}<div>{products.map((p)=><button key={p.id} className={p.id===modal.selectedId?"active":""} onClick={()=>setModal({...modal,selectedId:p.id})}><span>{file?.shelfImage?`POG ${file.line}${file.side}`:"POG chưa chuẩn"}</span><div><small>SKU {p.sku}</small><b>{p.name}</b><em>{p.supplierBarcode||p.barcode}</em></div><StockBadge stock={p.stock}/></button>)}{!products.length&&<div className="empty big">{file?.shelfImage?"Chưa có sản phẩm liên kết trong Master Data.":"POG cần được phân tích và ghép trước khi liên kết sản phẩm."}</div>}</div>{selected&&<section className="pog-selected"><div><b>{file?.shelfImage?`POG Line ${file.line}${file.side}`:"Chưa chuẩn hóa POG"}</b><span>{selected.name}</span><small>{file?.shelfImage?(linkedPositions.length?`${linkedPositions.length} vị trí POG: ${linkedPositions.map((position)=>position.number).join(", ")}`:"Chưa tìm thấy STT trên ảnh POG")+" · ":""}Tồn {selected.stock} · Loss {selected.loss} · HSD {selected.expDate||"chưa có"}</small></div><button disabled={selected.stock===0} onClick={()=>onPick(selected)}>+ Thêm vào đơn</button></section>}</aside><div className="pog-visual"><PogCanvas file={file} modal={modal} selected={selected} positions={linkedPositions} canUpload={canUpload} onReanalyze={onReanalyze} onUpload={()=>uploadRef.current?.click()}/><div className="pog-file-label">{file?file.fileName:`POG Line ${modal.line}${modal.side}`}</div></div>{search.trim()&&<section className="pog-mobile-results" aria-live="polite"><header><b>Thông tin sản phẩm</b><span>{total?`${total} kết quả`:`Không tìm thấy`}</span></header>{mobileProducts.length?<div>{mobileProducts.map((product)=>{const matches=positionsFor(product);return <article key={product.id} className={product.id===modal.selectedId?"active":""}><div className="pog-mobile-product-head"><div><small>SKU {product.sku}</small><b>{product.name}</b></div><StockBadge stock={product.stock}/></div><div className="pog-mobile-product-meta"><span>Barcode {product.supplierBarcode||product.barcode||"—"}</span><span>Line {file?.line||product.shelfLine||"—"}{file?.side||product.shelfSide||""}</span><span>{matches.length?`Vị trí ${matches.map((position)=>position.number).join(", ")}`:"Chưa có STT trên POG"}</span></div><div className="pog-mobile-product-actions"><button className="ghost" onClick={()=>setModal({...modal,selectedId:product.id})}>Khoanh trên ảnh</button><button className="primary" disabled={product.stock===0} onClick={()=>onPick(product)}>+ Thêm vào đơn</button></div></article>})}</div>:<div className="pog-mobile-empty">Không tìm thấy sản phẩm trong Master Data.</div>}{total>mobileProducts.length&&<small className="pog-mobile-more">Đang hiển thị {mobileProducts.length} kết quả đầu.</small>}</section>}</div>
