...
type DailyReport = {id:string;employeeName:string;date:string;phone:string;customerName:string;customerStatus:string;vatExport:string;orderType:string;invoiceNumber:string;invoiceValue:number|string;paymentMethod:string;cdoNumber:string;codNumber:string;carrier:string;returnStatus:string;remainingInvoiceValue:number|string;memberCard:string;customerGroup:string;email:string;taxId:string;vatAddress:string;deliveryAddress:string;note:string;createdAt?:number;updatedAt?:number;createdBy?:string};
    ...
    <main className="ops-shell">
      ...
      <div className="ops-body">
        ...
        <section className="ops-content">
          ...
          {tab==="DAILY_REPORT"&&<DailyReportView reports={dailyReports} customers={data.customers||[]} month={reportMonth} onMonth={setReportMonth} onSave={saveDailyReport} onImport={()=>customerExcelRef.current?.click()} onExport={exportDailyReports} onExportCustomers={exportCustomerMaster} canManage={canManage(data.actor.role)} busy={reportsBusy} purchaseHistory={purchaseHistory} purchaseMonth={purchaseMonth} onPurchaseMonth={setPurchaseMonth} onPurchaseImport={()=>purchaseExcelRef.current?.click()} onPurchaseExport={exportPurchaseHistory} purchaseBusy={purchaseBusy}/>}
...
function DailyReportHistoryView({reports,month,onEdit}:{reports:DailyReport[];month:string;onEdit:(report:DailyReport)=>void}) {
  return <section className="panel daily-report-list"><div className="panel-title"><div><h2>Lịch sử nhập báo cáo tháng {month}</h2><span>Bấm vào một dòng để mở lại báo cáo và chỉnh sửa.</span></div><b>{reports.length} báo cáo</b></div><div className="table-wrap"><table className="compact-table report-table"><thead><tr><th>Ngày</th><th>Tên NV</th><th>SĐT</th><th>Khách hàng</th><th>Số hóa đơn</th><th>Giá trị HĐ</th><th>PTTT</th><th>Loại đơn</th><th>Ghi chú</th></tr></thead><tbody>{reports.map((report)=><tr key={report.id} onClick={()=>onEdit(report)}><td>{report.date}</td><td>{report.employeeName}</td><td>{report.phone}</td><td><b>{report.customerName}</b><small>{report.customerGroup||""}</small></td><td>{report.invoiceNumber||"—"}</td><td>{report.invoiceValue?money.format(Number(report.invoiceValue)):"—"}</td><td>{report.paymentMethod||"—"}</td><td>{report.orderType||"—"}</td><td>{report.note||"—"}</td></tr>)}{!reports.length&&<tr><td colSpan={9}><div className="empty big"><b>Chưa có báo cáo trong tháng này</b><span>Các báo cáo bạn nhập sẽ được lưu và hiển thị tại đây.</span></div></td></tr>}</tbody></table></div></section>;
}
function PurchaseHistoryView({summary,month,onMonth,onImport,onExport,canManage,busy}:{summary:PurchaseHistorySummary;month:string;onMonth:(value:string)=>void;onImport:()=>void;onExport:(month:string)=>void;canManage:boolean;busy:boolean}) {
  const [query,setQuery]=useState("");
  const year=summary.year||month.slice(0,4),months=Array.from({length:12},(_,index)=>`${year}-${String(index+1).padStart(2,"0")}`),needle=normalize(query),customers=summary.customers.filter((customer)=>!needle||normalize(`${customer.phone} ${customer.customerName} ${customer.address}`).includes(needle));
  return <div className="purchase-history-view"><div className="purchase-history-toolbar"><label>Tháng và năm<input type="month" value={month} onChange={(event)=>onMonth(event.target.value)}/></label><label className="purchase-history-search">Tìm khách hàng<input type="search" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="SĐT, tên hoặc địa chỉ"/></label><button className="ghost" onClick={()=>onExport(month)}>↓ Xuất tổng hợp .xlsx</button><button className="primary" disabled={!canManage||busy} title={canManage?"Upload dữ liệu mua hàng của tháng đã chọn":"Cần quyền Manager hoặc Admin"} onClick={onImport}>{busy?<><i className="mini-spinner"/> Đang xử lý…</>:canManage?"↑ Upload dữ liệu tháng":"Cần quyền Manager/Admin"}</button></div><div className="purchase-history-note">Chọn đúng tháng/năm trước khi upload. Các dòng cùng số điện thoại sẽ được gộp thành một khách hàng; dữ liệu cũ của các tháng khác vẫn giữ nguyên trên server.</div><div className="purchase-history-cards"><article><small>Tổng tháng {month}</small><b>{money.format(summary.totals.monthValue)} đ</b><span>{summary.totals.monthOrders} dòng mua hàng</span></article><article><small>Tổng năm {year}</small><b>{money.format(summary.totals.yearValue)} đ</b><span>{summary.totals.yearOrders} dòng mua hàng</span></article><article><small>Khách hàng trong năm</small><b>{money.format(summary.totals.customerCount)}</b><span>Có thể tra cứu theo SĐT</span></article></div><section className="panel purchase-history-table-panel"><div className="panel-title"><div><h2>Tổng hợp theo khách hàng</h2><span>{customers.length} khách · tháng đang xem được tô màu</span></div><b>{busy?"Đang tải…":`${summary.records.length} dòng tháng ${month}`}</b></div><div className="table-wrap"><table className="compact-table purchase-history-table"><thead><tr><th>SĐT</th><th>Khách hàng</th><th>Địa chỉ</th>{months.map((item)=><th key={item} className={item===month?"current-period":""}>{item.slice(5)}</th>)}<th>Tổng năm</th></tr></thead><tbody>{customers.map((customer)=><tr key={`${customer.phone}-${customer.customerName}`}><td>{customer.phone||"—"}</td><td><b>{customer.customerName||"Chưa có tên"}</b></td><td>{customer.address||"—"}</td>{months.map((item)=><td key={item} className={item===month?"current-period":""}>{customer.monthlyTotals[item]?money.format(customer.monthlyTotals[item]):"—"}</td>)}<td><strong>{money.format(customer.yearTotal)}</strong></td></tr>)}{!customers.length&&<tr><td colSpan={16}><div className="empty big"><b>Chưa có dữ liệu lịch sử mua hàng</b><span>Upload file Excel theo tháng/năm để bắt đầu tổng hợp.</span></div></td></tr>}</tbody></table></div></section></div>;
}
function DailyReportView({reports,customers,month,onMonth,onSave,onImport,onExport,onExportCustomers,canManage,busy,purchaseHistory,purchaseMonth,onPurchaseMonth,onPurchaseImport,onPurchaseExport,purchaseBusy}:{reports:DailyReport[];customers:CustomerMaster[];month:string;onMonth:(value:string)=>void;onSave:(report:Record<string,unknown>)=>Promise<void>;onImport:()=>void;onExport:(month:string)=>void;onExportCustomers:()=>void;canManage:boolean;busy:boolean;purchaseHistory:PurchaseHistorySummary;purchaseMonth:string;onPurchaseMonth:(value:string)=>void;onPurchaseImport:()=>void;onPurchaseExport:(month:string)=>void;purchaseBusy:boolean}) {
  const blank=()=>({employeeName:"",date:todayOrderDate,phone:"",customerName:"",customerStatus:"",vatExport:"",orderType:"",invoiceNumber:"",invoiceValue:"",paymentMethod:"",cdoNumber:"",codNumber:"",carrier:"",returnStatus:"",remainingInvoiceValue:"",memberCard:"",customerGroup:"",email:"",taxId:"",vatAddress:"",deliveryAddress:"",note:"",id:""});
