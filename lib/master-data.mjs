...
function mapHeaderRow(row) {
  ...
  row.forEach((value,index)=>{
    ...
    if(supplierBarcodeHeaders.includes(key))indexes.supplierBarcode??=index;
    if(aeonBarcodeHeaders.includes(key))indexes.barcode??=index;
    if(key==="LINE"||key==="LINE CODE")indexes.line??=index;
    if(key==="LINE NAME")indexes.lineName??=index;
    if(salesHeaders.includes(key))indexes.sales??=index;
    if(stockHeaders.includes(key))indexes.stock??=index;
    if(retailPriceHeaders.includes(key))indexes.price??=index;
    if(promoPriceHeaders.includes(key))indexes.promoPrice??=index;
    if(imageHeaders.includes(key))indexes.imageUrl??=index;
  });
  return indexes;
}
