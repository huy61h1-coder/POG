...
export function mergeMasterRecords(products,records,{createId,now=Date.now()}={}) {
  ...
  for(const source of Array.isArray(records)?records:[]){
    ...
    const changed=requiredKeys.some((field)=>masterCellText(current?.[field])!==master[field])||masterCellText(current?.barcode)!==master.supplierBarcode||imageChanged;
    if(!changed){unchanged++;continue;}
    result[index]={...current,...master,imageUrl:incomingImage===undefined?normalizeImageUrl(current?.imageUrl):incomingImage,barcode:master.supplierBarcode,updatedAt:now};updated++;
  }
