...
export function parseStockRows(sheetRows,{maxRows=500000,onProgress}={}){
  ...
  let header={row:0,sku:-1,stock:-1,sales:-1};
  for(let rowIndex=0;rowIndex<Math.min(20,sheetRows.length);rowIndex++){
    const row=Array.isArray(sheetRows[rowIndex])?sheetRows[rowIndex]:[];let sku=-1,stock=-1,sales=-1;
    row.forEach((value,index)=>{const key=headerKey(value);if(sku<0&&["SKU","ITEM CODE","MA SAN PHAM","PRODUCT CODE"].includes(key))sku=index;if(stock<0&&["CLOSING STOCK","CLOSING INVENTORY","STOCK","STOCK QTY","TON KHO","TON","SO LUONG TON","ON HAND","INVENTORY","QTY","QUANTITY"].includes(key))stock=index;if(sales<0&&["SALE","SALES","SALE QTY","DOANH SO","SO LUONG BAN","SL BAN"].includes(key))sales=index;});
    if(sku>=0&&stock>=0){header={row:rowIndex,sku,stock,sales};break;}
  }
