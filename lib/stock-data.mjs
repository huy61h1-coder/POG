...
function headerKey(value){return masterCellText(value).normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[\u200B-\u200D\u2060\uFEFF]/g,"").toUpperCase().replace(/[^A-Z0-9]+/g," ").trim();}
function headerMatches(key,aliases){const compact=key.replace(/\s+/g,"");return aliases.some((alias)=>alias===key||alias.replace(/\s+/g,"")===compact);}
...
export function parseStockRows(sheetRows,{maxRows=500000,onProgress}={}){
  ...
  for(let rowIndex=0;rowIndex<Math.min(100,sheetRows.length);rowIndex++){
    ...
    row.forEach((value,index)=>{
      ...
      if(sku<0&&headerMatches(key,["SKU","ITEM CODE","MA SAN PHAM","PRODUCT CODE"]))sku=index;
      if(name<0&&headerMatches(key,["TEN SAN PHAM","PRODUCT NAME","ITEM NAME"]))name=index;
      if(stock<0&&headerMatches(key,["CLOSING STOCK","CLOSING INVENTORY","STOCK","STOCK QTY","TON KHO","TON","SO LUONG TON","ON HAND","INVENTORY","QTY","QUANTITY"]))stock=index;
      if(sales<0&&headerMatches(key,["SALE","SALES","SALE QTY","DOANH SO","SO LUONG BAN","SL BAN"]))sales=index;
      if(division<0&&headerMatches(key,["DIVISION","DIVISION CODE"]))division=index;
      if(divisionName<0&&headerMatches(key,["DIVISION NAME"]))divisionName=index;
      if(department<0&&headerMatches(key,["DEPARTMENT","DEPARTMENT CODE","DEPT CODE"]))department=index;
      if(departmentName<0&&headerMatches(key,["DEPARTMENT NAME","DEPT NAME"]))departmentName=index;
    });
