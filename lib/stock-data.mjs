...
export function parseStockRows(sheetRows,{maxRows=500000,onProgress}={}){
  ...
  let header={row:0,sku:-1,stock:-1};
  for(let rowIndex=0;rowIndex<Math.min(20,sheetRows.length);rowIndex++){
    const row=Array.isArray(sheetRows[rowIndex])?sheetRows[rowIndex]:[];let sku=-1,stock=-1;
    row.forEach((value,index)=>{const key=headerKey(value);if(sku<0&&["SKU","ITEM CODE","MA SAN PHAM","PRODUCT CODE"].includes(key))sku=index;if(stock<0&&["STOCK","TON KHO","SO LUONG TON","ON HAND","INVENTORY","QTY","QUANTITY"].includes(key))stock=index;});
    if(sku>=0&&stock>=0){header={row:rowIndex,sku,stock};break;}
  }
  if(header.sku<0||header.stock<0)throw new Error("File Stock cần có 2 cột: SKU và STOCK (hoặc TỒN KHO).");
  ...
  const bySku=new Map(),issues=[];let skipped=0,duplicates=0;
  rows.forEach((source,offset)=>{
    ...
    const sku=masterCellText(row[header.sku]),stock=stockNumber(row[header.stock]),rowNumber=header.row+offset+2;
    ...
    const key=sku.toUpperCase();if(bySku.has(key))duplicates++;bySku.set(key,{sku,stock});
  });
  ...
  return {records,headerRow:header.row+1,skipped,duplicates,issues};
}
