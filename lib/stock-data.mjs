...
export function parseStockRows(sheetRows,{maxRows=500000,onProgress}={}){
  ...
  for(let rowIndex=0;rowIndex<Math.min(20,sheetRows.length);rowIndex++){
    ...
    row.forEach((value,index)=>{const key=headerKey(value);if(sku<0&&["SKU","ITEM CODE","MA SAN PHAM","PRODUCT CODE"].includes(key))sku=index;if(stock<0&&["CLOSING STOCK","STOCK","TON KHO","SO LUONG TON","ON HAND","INVENTORY","QTY","QUANTITY"].includes(key))stock=index;if(sales<0&&["SALES","DOANH SO","SO LUONG BAN"].includes(key))sales=index;});
