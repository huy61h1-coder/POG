...
export async function readCustomerWorkbookSheets(source,{maxColumns=64,maxRows=200000}={}) {
  ...
  for(const match of workbookXml.matchAll(/<sheet\b([^>]*?)\/?\s*>/gi)){
    const sheet=attribute(match[1],"name"),relationshipId=attribute(match[1],"r:id"),target=relationships.get(relationshipId),xml=target?archiveText(archive,target):"";
