const aliases={
  ...
  memberCard:["THE THANH VIEN","MEMBER CARD","MA APP TV","APP TV"],
  customerGroup:["NHOM KH","NHOM KHACH HANG","CUSTOMER GROUP"],
  companyName:["TEN CONG TY","CONG TY","COMPANY NAME","COMPANY"],
  email:["EMAIL","E MAIL"],
...
export function customerFieldsFromReport(report){return {phone:text(report.phone),name:text(report.customerName),status:text(report.customerStatus),vatExport:text(report.vatExport),memberCard:text(report.memberCard),group:text(report.customerGroup),companyName:text(report.companyName),email:text(report.email),taxId:text(report.taxId),vatAddress:text(report.vatAddress),deliveryAddress:text(report.deliveryAddress)};}
